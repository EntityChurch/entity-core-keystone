...
#include "ec_shake256.h"   /* vendored SHAKE256 over keccak1600 (replaces EVP) */

/* ---------------------------------------------------------------------------
 * KEYSTONE FFI MODIFICATION (entity-core-codec-ffi-c): the upstream OpenSSL
 * eddsa.c reaches into the EVP/provider layer for SHAKE256. This build links no
 * OpenSSL library, so the EVP_MD_CTX hashing has been retargeted onto the
 * self-contained shake256 wrapper (same keccak1600 sponge, same pinned
 * source). The curve/field/scalar arithmetic below is byte-for-byte upstream.
 * The OSSL_LIB_CTX *ctx and const char *propq parameters are retained in every
 * signature but ignored (always passed NULL). See src/ed448/PROVENANCE.md.
 * ------------------------------------------------------------------------- */
...
static c448_error_t hash_init_with_dom(OSSL_LIB_CTX *ctx, shake256_ctx *hashctx,
                                       uint8_t prehashed,
                                       uint8_t for_prehash,
                                       ...
                                       const char *propq)
{
    ...
    shake256_init(hashctx);
    shake256_update(hashctx, dom_s, sizeof(dom_s) - 1);
    shake256_update(hashctx, dom, sizeof(dom));
    shake256_update(hashctx, context, context_len);
    return C448_SUCCESS;
}
...
{
    curve448_scalar_t secret_scalar;
    shake256_ctx *hashctx = shake256_new();
...

    {
        ...
        if (!hash_init_with_dom(ctx, hashctx, prehashed, 0, context,
                                context_len, propq)
                || !shake256_update(hashctx,
                                     expanded + EDDSA_448_PRIVATE_BYTES,
                                     EDDSA_448_PRIVATE_BYTES)
                || !shake256_update(hashctx, message, message_len)) {
    ...
    /* Decode the nonce */
    {
        ...
        if (!shake256_final_xof(hashctx, nonce, sizeof(nonce)))
            goto err;
...

    {
        ...
        if (!hash_init_with_dom(ctx, hashctx, prehashed, 0, context, context_len,
                                propq)
                || !shake256_update(hashctx, nonce_point, sizeof(nonce_point))
                || !shake256_update(hashctx, pubkey, EDDSA_448_PUBLIC_BYTES)
                || !shake256_update(hashctx, message, message_len)
                || !shake256_final_xof(hashctx, challenge, sizeof(challenge)))
            goto err;
...

    {
        ...
        if (hashctx == NULL
                || !hash_init_with_dom(ctx, hashctx, prehashed, 0, context,
                                       context_len, propq)
                || !shake256_update(hashctx, signature, EDDSA_448_PUBLIC_BYTES)
                || !shake256_update(hashctx, pubkey, EDDSA_448_PUBLIC_BYTES)
                || !shake256_update(hashctx, message, message_len)
                || !shake256_final_xof(hashctx, challenge, sizeof(challenge))) {
            shake256_free(hashctx);
