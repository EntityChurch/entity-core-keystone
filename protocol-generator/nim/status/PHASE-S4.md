...
| `src/types.nim` (new) | The 53-type §9.5 floor rendered NATIVELY (TypeDef/FSpec model → the peer's own ECF codec, omit-empty), seeded at `/{peer}/system/type/<name>`. All 53 `_fetch`+`_match` byte-identical to the Go registry (drift target `shared/test-vectors/v0.8.0/type-registry-vectors-v1.diag`). |
