#!/usr/bin/env bash
# §10.2 origination-core probe — Common Lisp target (A-role) against the Go
# entity-peer reference (B-role). Both run inside common-lisp-toolchain (Go ELFs
# run there); shared loopback, sealed-offline (--network=none).
#
...
#
# CL note: the §6.11 outbound/reentry seam is the native sb-thread build
# (peer-transport.lisp reader-demux + per-thread dispatch + outbound primitive in
# peer.lisp:outbound-dispatch); this gate is the cross-impl wire proof of it.
#
# Invoke from the repo root:
#   ./protocol-generator/common-lisp/run-origination-core.sh
set -euo pipefail
...
TPORT="${TPORT:-7777}"   # target (Common Lisp)
...
podman run $PODMAN_RUN_CAPS --rm --network=none -v "$REPO_ROOT":/work:Z -w "$WORKDIR" "$IMAGE" \
  bash -c '
    ...
    REF_PID=$!
    # Target CL host (A-role) — --validate makes system/validate/dispatch-outbound live.
    ...
    i=0; while [ "$i" -lt 200 ]; do
      ...
      kill -0 "$HOST_PID" 2>/dev/null || { echo "CL host exited:"; cat /tmp/host.err >&2; exit 1; }
      i=$((i+1)); sleep 0.1
    done
    sleep 1   # give Go reference a moment to bind
    echo "target(CL)=$(head -1 /tmp/host.out)  reference(Go) on :$RPORT"
