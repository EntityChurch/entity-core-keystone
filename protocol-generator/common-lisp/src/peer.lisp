...
;; scoped "in any state", so this method covers pre-handshake AND established;
;; the genuine sequence cases are refused in the :hello/:authenticate methods.
;;
;; SCOPED TO THIS HANDLER DELIBERATELY, and CLOS specificity is what scopes it:
;; this method is more specific than the `(h handler)` default, so the generic
;; registered-handler rule (§3.3's 501 row, §6.2) is untouched. Moving that
;; shared default would trade one green check for another.
(defmethod handle-op ((h connect-handler) op ctx)
  (declare (ignore ctx))
  (err 400 "invalid_request" (format nil "connect: unknown operation ~a" op)))
  ...
  (let ((peer (handler-peer h)) (conn (ctx-conn ctx)) (exec (ctx-exec ctx)))
    (if (conn-established conn)
        ...
        (let ((hash-ok (let ((f (str-array exec "hash_formats")))
              ...
              ;; refuses it — `ignore-errors` yields NIL, read as "not our question".
              (initiator-bad-kt
                (let ((pid (let ((p (entity-entity exec "params")))
                             (and p (entity-text p "peer_id")))))
                  (and pid (ignore-errors
                            (multiple-value-bind (kt) (peer-id-parse pid)
                              (/= kt 1))))))
              (protos (str-array exec "protocols")))
          (cond
            ;; §4.7 out-of-order row + the 0.8.2.8 half-open note: a second hello
            ;; on a HALF-OPEN connection (hello done, authenticate not yet) is an
            ;; operation we implement arriving in a state that forbids it — the
            ;; same class as connection_already_established above, taking the same
            ;; 409. A half-open connection is NOT established, so the guard above
            ;; cannot reach it; §4.7 names this gap explicitly because two adjacent
            ;; rules each look like they cover it and neither does.
            ((conn-issued-nonce conn) (err 409 "connection_sequence_error"))
            ((not hash-ok) (err 400 "incompatible_hash_format"))
            ((not key-ok) (err 400 "unsupported_key_type"))
            (initiator-bad-kt (err 400 "unsupported_key_type"))
            ...
            ;; no precedence between the three, so a hello disjoint in more than one
            ...
            ;; identifier (F56).
            ((null protos) (err 400 "invalid_request" "hello: protocols absent or empty"))
            ((not (member "entity-core/1.0" protos :test #'string=))
