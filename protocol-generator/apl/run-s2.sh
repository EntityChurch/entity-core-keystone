...
# apl-toolchain image), compiles the GNU APL native-function shim, and runs the pinned
# v0.8.0 corpus gate (69/69 byte-identical) via the hand-rolled pure-APL array codec +
# the C-ABI crypto/base58 floor. Everything is offline: the codec/varint/harness are
# pure APL in-repo; crypto/base58 ride libentitycore_codec (built from the vendored
# C-ABI, links the image libsodium).
#
#   ./run-s2.sh          # full gate: make test  (69-vector corpus + unit suite)
