#!/bin/sh
# §10.2 origination-core probe — Zig target (A-role) against the Go entity-peer
# reference (B-role). Both run inside the zig-toolchain container (Go ELFs run
# there); shared loopback, sealed-offline with --network=none. Post-§7a resolution
# (2026-06-13): runOriginationCore is the dispatch_outbound_reentry probe — the
# validator mints a reentry capability, EXECUTEs system/validate/dispatch-outbound
# on the target, and the target originates an outbound EXECUTE back to the
# validator-as-B over the SAME inbound connection (§6.11 reentry; NOT a fresh dial
# to the reference). The reference is connected only to keep the gate's input
# shape (`-reference-peer required`) consistent with --profile full; otherwise
# unused under core. The target MUST run with --validate (dispatch-outbound live);
# absent it, the probe honest-SKIPs.
#
# Zig note: the §6.11 reentry seam is the transport.zig reader-demux + the
# OutboundFn ABI shim (transport.zig "§6.11 reentry shim"). This gate is the
# cross-impl wire proof of that seam from the no-GC systems idiom.
#
# Invoke from the repo root:
#   podman run --memory=4g --memory-swap=4g --pids-limit=2048 --cpus=4 --rm --network=none -v "$PWD":/work:Z \
#     entity-core-keystone/zig-toolchain:latest sh /work/protocol-generator/zig/run-origination-core.sh
set -eu

# ── Host entry point ────────────────────────────────────────────────────────
# This script was inside-container ONLY until 2026-09-02: invoked from the host
# it died with `cd: /work/...: No such file or directory`, which reads as a
# broken tree rather than as a wrong invocation. The origination axis had no
# cohort sweep, so nothing ever invoked it the way a sweep does, and FOURTEEN of
# the thirty-one authored gates were in this state simultaneously. The podman
# line below is the one this file's own header already documented — it is now
# executed instead of described. Set INCONTAINER=1 to skip the relaunch.
if [ "${INCONTAINER:-0}" != "1" ]; then
  HOSTREPO="$(cd "$(dirname "$0")/../.." && pwd)"
  . "$HOSTREPO/tools/podman-caps.sh"
  exec podman run $PODMAN_RUN_CAPS --rm --network=none -e INCONTAINER=1 \
    -v "$HOSTREPO":/work:Z -w /work entity-core-keystone/zig-toolchain:latest \
    sh /work/protocol-generator/zig/run-origination-core.sh "$@"
fi

TPORT="${TPORT:-7777}"   # target (Zig)
RPORT="${RPORT:-7778}"   # reference (Go)
ORACLE="${ORACLE:-/work/output/s4-oracles/validate-peer}"
REFPEER="${REFPEER:-/work/output/s4-oracles/entity-peer}"
PROJ=/work/protocol-generator/zig
cd "$PROJ"

[ "${NOBUILD:-0}" = "1" ] || zig build -Doptimize=ReleaseSafe >/dev/null

# Reference Go peer (B-role), open-access.
"$REFPEER" --addr "127.0.0.1:$RPORT" --open-access >/tmp/ref.out 2>/tmp/ref.err &
REF_PID=$!
# Target Zig host (A-role) — --validate makes system/validate/dispatch-outbound live.
./zig-out/bin/host --port "$TPORT" --debug-open-grants --validate >/tmp/host.out 2>/tmp/host.err &
HOST_PID=$!
trap 'kill "$HOST_PID" "$REF_PID" 2>/dev/null || true' EXIT INT TERM

i=0; while [ "$i" -lt 200 ]; do
  grep -q '^LISTENING' /tmp/host.out 2>/dev/null && break
  kill -0 "$HOST_PID" 2>/dev/null || { echo "Zig host exited:"; cat /tmp/host.err >&2; exit 1; }
  i=$((i+1)); sleep 0.1
done
sleep 1   # give Go reference a moment to bind
echo "target(Zig)=$(head -1 /tmp/host.out)  reference(Go) on :$RPORT"

"$ORACLE" -addr "127.0.0.1:$TPORT" -reference-peer "127.0.0.1:$RPORT" \
  -profile core -category origination "$@" || true
echo "=== Go reference stderr (tail) ===" ; tail -5 /tmp/ref.err 2>/dev/null || true
