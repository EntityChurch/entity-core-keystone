#!/bin/sh
# §10.2 origination-core probe — Swift target (A-role) against the Go entity-peer
# reference (B-role). Both run inside swift-toolchain (Go ELFs run there); shared
# loopback, sealed-offline with --network=none. Post-§7a resolution (2026-06-13):
...
# MUST run with --validate (dispatch-outbound live); absent it, the probe honest-SKIPs.
#
# Swift note: the §6.11 reentry seam is the Transport.swift Connection actor's
# reader-demux + the `makeOutbound`/`originate` closure (the §6.13b outbound shim).
# This gate is the cross-impl wire proof of that seam from the actor-concurrency idiom.
#
...
if [ "${INCONTAINER:-0}" != "1" ]; then
  ...
  exec podman run $PODMAN_RUN_CAPS --rm --network=none -e INCONTAINER=1 \
    -v "$HOSTREPO":/work:Z -w /work entity-core-keystone/swift-toolchain:latest \
    sh /work/protocol-generator/swift/run-origination-core.sh "$@"
fi

TPORT="${TPORT:-7777}"   # target (Swift)
...
[ "${NOBUILD:-0}" = "1" ] || swift build -c release >/dev/null

# Reference Go peer (B-role), open-access.
"$REFPEER" --addr "127.0.0.1:$RPORT" --open-access >/tmp/ref.out 2>/tmp/ref.err &
REF_PID=$!
# Target Swift host (A-role) — --validate makes system/validate/dispatch-outbound live.
./.build/release/entity-peer-swift --port "$TPORT" --debug-open-grants --validate >/tmp/host.out 2>/tmp/host.err &
HOST_PID=$!
trap 'kill "$HOST_PID" "$REF_PID" 2>/dev/null || true' EXIT INT TERM

i=0; while [ "$i" -lt 200 ]; do
  grep -q '^LISTENING' /tmp/host.out 2>/dev/null && break
  kill -0 "$HOST_PID" 2>/dev/null || { echo "Swift host exited:"; cat /tmp/host.err >&2; exit 1; }
  i=$((i+1)); sleep 0.1
done
sleep 1   # give Go reference a moment to bind
echo "target(Swift)=$(head -1 /tmp/host.out)  reference(Go) on :$RPORT"
