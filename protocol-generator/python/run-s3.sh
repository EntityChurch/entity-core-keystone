#!/usr/bin/env bash
# S3 peer machinery — the peer test package (tests/peer) — dispatch + handshake over loopback.
#
...
podman run $PODMAN_RUN_CAPS --rm --network=none \
  ...
  bash -lc 'cd /work/protocol-generator/python && python -m pytest -q tests/peer'
