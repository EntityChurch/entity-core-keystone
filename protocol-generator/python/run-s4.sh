...
podman run $PODMAN_RUN_CAPS --rm --network=none -v "$REPO_ROOT":/work:Z -w "$WORKDIR" "$IMAGE" \
  bash -c '
    ...
    # Forward the CALLER ARGS. This line used to hardcode `-profile core -json-out
    # "$JSON_OUT"` and drop "$@" on the floor, so `run-s4.sh -category connectivity`
    # silently ran the whole 756-check suite AND rewrote the tracked, signed-off
    # CONFORMANCE-REPORT.json -- a diagnostic run overwriting the record it is
    # supposed to be diagnosed against. 38 of 46 peers already had this shape; the
    # trailing `bash "$@"` after the -c script is what carries argv across the podman
    # boundary with its quoting intact (sql/io/pd/datalog already did it that way --
    # the word bash is argv[0], and the caller args land as $1.. inside the block).
