...
def verify_capability_chain(
    ...
    """
    return verify_capability_chain_rooted_at(local_peer, local_peer, store, capability, included)


def verify_capability_chain_rooted_at(
    local_peer: str, root_peer: str, store: Store, capability: Entity, included: dict
) -> Verdict:
    """:func:`verify_capability_chain` with the expected ROOT granter named separately
    ...
    LOCAL peer in the signer set, so the quorum arm has no meaning in a foreign frame
    ...
    if _granter_is_multi(root):
        # §1.4 (0.8.2.19): a multi-signature root NEVER relaxes Dimension 4, so it is
        # only ever valid in the LOCAL frame. Refuse outright rather than verify.
        if root_peer != local_peer:
            return AUTHZ_DENY
        ...
        if pk is None or peer_id_of_public_key(pk) != root_peer:
            return AUTHZ_DENY
...
def peer_relative_of(local_peer: str, uri: str) -> str:
    ...
    if not p.startswith("/"):
        return p
    segs = p.split("/")  # leading "" from the absolute form
    if len(segs) >= 3 and segs[0] == "" and is_peer_id(segs[1]):
        return "/".join(segs[2:])
...
def grant_path_for(local_peer: str, pattern: str) -> str:
    ...
    return "/" + local_peer + "/system/capability/grants/" + pattern


def target_minted_peers_relaxation(
    local_peer: str, target_peer: str, store: Store, cred: Entity, included: dict
) -> Scope | None:
    ...
    * the chain ROOT ``granter`` resolves to the TARGET peer, and is NOT a
      ...
      Dimension 4 (:func:`verify_capability_chain_rooted_at` refuses the quorum arm in a
    ...
    * valid (per-link signatures, temporal, attenuation, caveats) and not revoked.
    ...
    if target_peer == local_peer:
        ...
        return None
    if verify_capability_chain_rooted_at(local_peer, target_peer, store, cred, included) != ALLOW:
        return None
    if _is_revoked(local_peer, store, cred, included):
        return None
    geh = cred.bytes_("grantee")
    if geh is None:
        return None
    ge = cap_resolve(included, store)(geh)
    if ge is None:
        return None
    pk = ge.bytes_("public_key")
    if pk is None or peer_id_of_public_key(pk) != local_peer:
        return None
    ...
    # dispatch back to me."
    for g in _grants_of_token(cred):
        return g.peers if g.peers is not None else Scope([target_peer], [])
    return None
...
def check_outbound_sub_dispatch(
    ...
    *"The target answers WHERE; the handler's grant answers WHAT."*  A credential is NOT
    ...
    ``cred is None`` is the ambient arm: Dimension 4 is decided by the handler's grant
    alone.
    """
    # Computed FIRST and consulted LAST, so no credential can stand in for 1-3.
    relax_to: Scope | None = None
    if cred is not None:
        relax_to = target_minted_peers_relaxation(local_peer, target_peer, store, cred, included)

    for g in _grants_of_token(handler_grant):
        if not _matches_scope(local_peer, handler_pattern, g.handlers, "path"):
            continue
        if not _matches_scope(local_peer, operation, g.operations, "id"):
            continue
        if not _check_resource_scope(local_peer, local_peer, resource, g.resources):
            continue
        ...
        peers = g.peers if g.peers is not None else Scope([local_peer], [])
        if _matches_scope(local_peer, target_peer, peers, "id"):
            return True
        if relax_to is not None and _matches_scope(local_peer, target_peer, relax_to, "id"):
            return True
    return False
