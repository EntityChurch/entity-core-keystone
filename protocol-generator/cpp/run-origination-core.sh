...
# reference (B-role). Both run inside cpp-toolchain (Go ELFs run there); shared
...
if [ "${INCONTAINER:-0}" != "1" ]; then
  ...
  exec podman run $PODMAN_RUN_CAPS --rm --network=none -e INCONTAINER=1 \
    -v "$HOSTREPO":/work:Z -w /work entity-core-keystone/cpp-toolchain:latest \
    sh /work/protocol-generator/cpp/run-origination-core.sh "$@"
fi
...
"$BUILD/host" --port "$TPORT" --debug-open-grants --validate >/tmp/host.out 2>/tmp/host.err &
HOST_PID=$!
