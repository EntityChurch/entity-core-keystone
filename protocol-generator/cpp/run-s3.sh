#!/usr/bin/env bash
# S3 peer machinery — the two-peer loopback smoke (test/smoke.cpp, ctest target `smoke`).
#
...
podman run $PODMAN_RUN_CAPS --rm --network=none \
  ...
  bash -lc 'cd /work/protocol-generator/cpp && cmake -S . -B /tmp/s3 -DCMAKE_BUILD_TYPE=Release >/dev/null && cmake --build /tmp/s3 -j4 >/dev/null && ctest --test-dir /tmp/s3 -R '^smoke$' --output-on-failure'
