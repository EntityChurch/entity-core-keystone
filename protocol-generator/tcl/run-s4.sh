#!/bin/sh
# S4 conformance harness — entity-core-protocol-tcl.
#
# Runs entirely inside the tcl-toolchain container (the Go validate-peer oracle is a
# fedora:43 ELF binary that runs there too, so oracle + peer share one loopback and
# the run stays sealed-offline: --network=none). No compile step for the peer itself
# (pure Tcl); only the crypto C-extension shim is prebuilt (`make shim`, which also
# builds libentitycore_codec if absent). Launches the peer via bin/peer.tcl with
...
#     -v "$PWD":/work:Z entity-core-keystone/tcl-toolchain:latest sh /work/protocol-generator/tcl/run-s4.sh [validate-peer-args...]
#
...
[ -x "$ORACLE_HOST" ] || { echo "run-s4: ERROR conformance oracle not found at $ORACLE_HOST" >&2
  ...
  exit 3; }
PROJ=/work/protocol-generator/tcl

cd "$PROJ"

# The crypto shim must be present (+ libentitycore_codec it links). Build if absent.
make shim >/tmp/shim.out 2>&1 || { echo "shim build failed:" >&2; cat /tmp/shim.out >&2; exit 1; }
...
# the launcher default, so peer_id (2KHoAk…) is unchanged. NAME follows the Go
...
LD_LIBRARY_PATH="$CODEC_BUILD" tclsh "$PROJ/bin/peer.tcl" --port "$PORT" --name "$NAME" \
...
# Wait up to 15s for the readiness line (Tcl startup + source graph + shim load).
i=0
while [ "$i" -lt 150 ]; do
...
if [ "$#" -eq 0 ]; then
  set -- -profile core -json-out "${JSON_OUT:-/tmp/ec-s4-tcl.json}"
fi
