...
module EntityCore
  ...
  class Peer
    ...
    class ConnectHandler < Handler
      ...
      # 400 invalid_request, not the 501 the base Handler answers. The table
      ...
      # scoped "in any state", so this covers pre-handshake AND established; the
      # genuine sequence cases are refused in op_hello/op_authenticate, with 409.
      #
      # SCOPED TO THIS HANDLER DELIBERATELY, and the override is what scopes it:
      # the generic registered-handler rule (§3.3's 501 row, §6.2) lives in
      # Handler#handle and is a different contract, separately gated. Moving that
      # shared 501 would trade one green check for another.
      def handle(operation, ctx)
        unless self.class.ops.include?(operation)
      ...
      def op_hello(ctx)
        ...
        if initiator
          begin
            hello_kt, = PeerId.parse(initiator)
            return Outcome.err(400, "unsupported_key_type") unless hello_kt == 1
        ...
        return Outcome.err(400, "invalid_request", "hello: protocols absent or empty") if protos.nil? || protos.empty?
        return Outcome.err(400, "incompatible_protocol") unless protos.include?("entity-core/1.0")
    ...
    class HandlersHandler < Handler
      ...
      def op_register(ctx)
        ...
        if pattern == "system" || pattern.start_with?("system/")
