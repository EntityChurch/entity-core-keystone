#!/usr/bin/env bash
# run-s2.sh — S2 (codec / crypto-agility) gate for the rust-wasm peer.
#
# THIS PEER HAS NO CODEC OF ITS OWN, AND THAT IS A CLAIM THIS SCRIPT CHECKS RATHER THAN
# ASSERTS. It is a thin wasm transport seam over `rust`: Cargo.toml takes the native Rust peer's
# library as an unmodified path dependency and cross-compiles it to wasm.
...
echo "=============================================================="
echo " S2 codec gate — entity-core-protocol-rust-wasm"
...
if ! grep -q 'entity-core-protocol-rust = { path = "../rust" }' "$PEER/Cargo.toml"; then
  echo "    RED: rust-wasm no longer path-depends on ../rust unmodified."
