#!/usr/bin/env bash
# S3 peer machinery — the S3 selftest (test/selftest.exe).
#
...
podman run $PODMAN_RUN_CAPS --rm --network=none \
  -v "$REPO_ROOT":/work:Z \
  entity-core-keystone/ocaml-toolchain:latest \
  bash -lc 'cd /work/protocol-generator/ocaml && dune build test/selftest.exe 2>&1 && dune exec --no-build test/selftest.exe'
