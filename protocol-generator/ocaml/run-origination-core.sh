#!/bin/sh
# §10.2 origination-core probe — OCaml target (A-role) against the Go entity-peer
# reference (B-role). Both run inside ocaml-toolchain (Go ELFs run there); shared
# loopback. Post-§7a resolution (2026-06-13): runOriginationCore is the
...
#
# OCaml note: its §6.11 outbound/reentry seam was a from-zero build (transport.ml
# reader-demux + per-thread dispatch + outbound primitive); this gate is the
# cross-impl wire proof of that seam.
set -eu
...
if [ "${INCONTAINER:-0}" != "1" ]; then
  ...
  exec podman run $PODMAN_RUN_CAPS --rm --network=none -e INCONTAINER=1 \
    -v "$HOSTREPO":/work:Z -w /work entity-core-keystone/ocaml-toolchain:latest \
    sh /work/protocol-generator/ocaml/run-origination-core.sh "$@"
fi

TPORT="${TPORT:-7777}"   # target (OCaml)
...
[ "${NOBUILD:-0}" = "1" ] || dune build

# Reference Go peer (B-role), open-access.
"$REFPEER" --addr "127.0.0.1:$RPORT" --open-access >/tmp/ref.out 2>/tmp/ref.err &
REF_PID=$!
# Target OCaml host (A-role) — --validate makes system/validate/dispatch-outbound live.
dune exec bin/host.exe -- --port "$TPORT" --debug-open-grants --validate >/tmp/host.out 2>/tmp/host.err &
HOST_PID=$!
trap 'kill "$HOST_PID" "$REF_PID" 2>/dev/null || true' EXIT INT TERM

i=0; while [ "$i" -lt 200 ]; do
  grep -q '^LISTENING' /tmp/host.out 2>/dev/null && break
  kill -0 "$HOST_PID" 2>/dev/null || { echo "OCaml host exited:"; cat /tmp/host.err >&2; exit 1; }
  i=$((i+1)); sleep 0.1
done
sleep 1   # give Go reference a moment to bind
echo "target(OCaml)=$(head -1 /tmp/host.out)  reference(Go) on :$RPORT"
