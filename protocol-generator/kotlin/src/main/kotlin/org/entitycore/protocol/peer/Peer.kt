 ...
 */
/**
 * The outcome of §6.3's `put` admission ladder — exactly one of [entity] /
 * [refusal] is non-null. File-private and top-level because Kotlin does not
 * allow a nested class inside an `inner class`, which is where the ladder runs.
 */
private class PutAdmission(val entity: Entity?, val refusal: Outcome?)
...
class Peer private constructor(
    ...
    private inner class ConnectHandler : Handler {
        override suspend fun handle(operation: String, ctx: HandlerContext): Outcome = when (operation) {
            ...
            else -> Outcome.err(400, "invalid_request", "connect: unknown operation $operation")
        }
        ...
        private fun hello(ctx: HandlerContext): Outcome {
            ...
            if (conn.issuedNonce != null) return Outcome.err(409, "connection_sequence_error")
            ...
            val helloPid = params?.text("peer_id")
            if (helloPid != null) {
                val badKt = try {
                    PeerId.parse(helloPid).keyType != PeerId.KEY_TYPE_ED25519
                } catch (_: Exception) {
                    false
                }
            ...
            val protos = strArray(exec, "protocols")
            if (protos.isNullOrEmpty()) {
                return Outcome.err(400, "invalid_request", "hello: protocols absent or empty")
            }
            if (!protos.contains("entity-core/1.0")) return Outcome.err(400, "incompatible_protocol")
    ...
    private inner class TreeHandler : Handler {
         ...
         */
        private fun hashDigestLen(formatCode: Long): Int? = when (formatCode) {
            0x00L -> 32
            0x01L -> 48
            else -> null
        }
         ...
         */
        private fun admitPut(v: EcfValue): PutAdmission {
            fun refuse(code: String, message: String): PutAdmission =
                PutAdmission(null, Outcome.err(400, code, message))

            if (v !is EcfValue.MapVal) {
                return refuse("invalid_request", "put: entity is not a map")
            }
            val typeV = v["type"]
            if (typeV !is EcfValue.Text || typeV.value.isEmpty()) {
            ...
            val chV = v["content_hash"]
            if (chV !is EcfValue.Bytes || chV.octets().isEmpty()) {
                return refuse("invalid_request", "put: entity.content_hash absent or not a byte string")
            }
            val carried = chV.octets()
            val decoded = try {
                Varint.decode(carried, 0)
            } catch (e: Exception) {
        ...
        private fun put(ctx: HandlerContext): Outcome {
            ...
            if (rawEntity == null) return Outcome.err(400, "unexpected_params", "put: missing entity")
            val admission = admitPut(rawEntity)
            admission.refusal?.let { return it }
            val entity = admission.entity!!
