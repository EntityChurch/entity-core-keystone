#!/usr/bin/env bash
# S3 peer machinery — the peer spec (spec/peer_spec.cr) — the two-peer dispatch surface.
#
...
podman run $PODMAN_RUN_CAPS --rm --network=none \
  ...
  bash -lc 'cd /work/protocol-generator/crystal && crystal spec spec/peer_spec.cr'
