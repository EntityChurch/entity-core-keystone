...
.Ld_notresp:
	...
	test %rax, %rax
	jnz  .Ld_ret                     # foreign address → 400 invalid_request already sent
	# operation
	mov  %rbx, %rdi
	lea  k_op(%rip), %rsi
	mov  $9, %rdx
...
# =====================================================================
# admit_put(rdi = submitted entity value) -> rax: 0 admitted · 1 invalid_request
...
admit_put:
	...
	test %rdx, %rdx
	jz   .Lap_invreq
	mov  %rax, %r12                   # type bytes ptr
	mov  %rdx, %r13                   # type len
	...
	test %rax, %rax
	jz   .Lap_invreq
	mov  %rax, %r14                   # data value ptr
	mov  %rax, %rdi
	call skip_value
	sub  %r14, %rax
	mov  %rax, %r15                   # data len
	...
	test %rdx, %rdx
	jz   .Lap_invreq
	mov  %rax, %rbx                   # carried bytes ptr (entity ptr no longer needed)
	mov  %rdx, %rcx                   # carried len
...
.Lap_vd:
	cmp  %rcx, %r10
	jae  .Lap_vdend
	movzbq (%rbx,%r10,1), %rax
	mov  %rax, %rdx
	and  $0x7f, %rdx
	mov  %r9, %rsi                    # shl needs cl; stash shift
	push %rcx
	mov  %rsi, %rcx
	shl  %cl, %rdx
	pop  %rcx
...
.Lap_vdend:
	...
	jz   .Lap_invreq
	test %r8, %r8
	jnz  .Lap_unsupfmt                # §1.2 / §4.7 row 5 — shape fine, algorithm absent
	mov  %r10, %rax
	add  $32, %rax
	cmp  %rax, %rcx
	jne  .Lap_invreq
...
.Lstp_unsupfmt:
	mov  $400, %rdi
	lea  ec_unsupported_chf(%rip), %rsi
	call send_error
	jmp  .Lstp_done
.Lstp_hashmm:
	mov  $400, %rdi
...
# =====================================================================
# uri_targets_local(rdi = exec data map) -> rax = 0 local / 1 foreign (400 already sent).
#
...
# those passes the gate untouched — hello/authenticate carry no uri and are unaffected. Only
# a first segment that is_peer_id accepts can ever differ from ours, which is what makes
# this a gate on a real foreign address rather than on a malformed one.
	.type uri_targets_local, @function
uri_targets_local:
	push %rbx                        # 1 push (odd) → 16B-align the send_error call
	call derive_handler              # rdi = exec (→ g_target_peer_ptr/len)
	mov  g_target_peer_len(%rip), %rax
	cmp  g_peerid_len(%rip), %rax
	jne  .Lutl_foreign               # lengths differ → not us
