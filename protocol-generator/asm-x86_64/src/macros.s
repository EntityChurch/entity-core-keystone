...
# arm64/riscv ports are a mechanical number swap — see arch/OPTIONS-AND-ISA-MAP.md.
	...
	# Bound on simultaneously-live connection children. The parent counts forks and
	# reaps; over the bound it closes the accepted fd immediately rather than forking.
	# Chosen against the substrate, not the check: each child COWs a 16 MiB b_req plus
	# its seeded store, so an unbounded fork-per-connection peer converts a connection
