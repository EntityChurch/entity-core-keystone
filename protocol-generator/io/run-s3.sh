#!/usr/bin/env bash
# S3 peer machinery — the two-peer loopback smoke (test/smoke.io + src/smoke-peer.io).
#
...
podman run $PODMAN_RUN_CAPS --rm --network=none \
  -v "$REPO_ROOT":/work:Z \
  entity-core-keystone/io-toolchain:latest \
  bash -lc 'cd /work/protocol-generator/io && make smoke'
