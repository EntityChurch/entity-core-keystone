#!/usr/bin/env bash
# entity-core-protocol-smalltalk — S3 peer machinery gate: the peer-layer foundation self-test
...
# gate stays dependency-sealed and offline (no reference peer needed; Pharo owns the sockets +
# the single-event-loop + crypto in-process via UFFI — A-ST native_sockets / in_process_ffi;
# NO co-process daemon, contrast Rexx).
#
