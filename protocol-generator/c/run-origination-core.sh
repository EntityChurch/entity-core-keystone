#!/usr/bin/env bash
# §10.2 origination-core probe — C target (A-role) against the Go entity-peer reference
# (B-role). Both run inside c-toolchain (Go ELFs run there); shared loopback, sealed-offline
# (--network=none).
#
...
# otherwise unused under core. The target MUST run with --validate (system/validate/dispatch-
# outbound live); absent it the probe honest-SKIPs — which is why the single-peer run-s4.sh
# honest-SKIPs origination.
#
# C note: the §6.11 outbound/reentry seam is the per-connection reader thread + request_id
# demux (transport.c) + the outbound primitive ec_io_outbound; dispatch-outbound is a
# *generic relay* — it forwards the {value:X} params bytes verbatim and returns the
# downstream result entity verbatim (RULINGS-CONCURRENCY-GATE-7b-MATRIX-2026-06-13 #2).
#
# Invoke from the repo root:
#   ./protocol-generator/c/run-origination-core.sh
set -euo pipefail
...
podman run $PODMAN_RUN_CAPS --rm --network=none -v "$REPO_ROOT":/work:Z -w "$WORKDIR" "$IMAGE" \
  bash -c '
    ...
    REF_PID=$!
    # Target C host (A-role) — --validate makes system/validate/dispatch-outbound live.
    ./entity-peer-c --port "$TPORT" --debug-open-grants --validate >/tmp/host.out 2>/tmp/host.err &
    HOST_PID=$!
    trap "kill $HOST_PID $REF_PID 2>/dev/null || true" EXIT INT TERM
    i=0; while [ "$i" -lt 300 ]; do
      grep -q "^LISTENING" /tmp/host.out 2>/dev/null && break
      kill -0 "$HOST_PID" 2>/dev/null || { echo "C host exited:"; cat /tmp/host.err >&2; exit 1; }
      i=$((i+1)); sleep 0.1
    done
    sleep 1   # give Go reference a moment to bind
    echo "target(C)=$(head -1 /tmp/host.out)  reference(Go) on :$RPORT"
