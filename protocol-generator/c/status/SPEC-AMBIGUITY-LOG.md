...
| **A-C-008** cohort scorecard oracle label `62044c5` is off-by-one; `b30a589` is the true v7.75 oracle | **⚑ provenance** | **mainline/arch** | **RESOLVED (in-repo)** — verified read-only; 0-FAIL at the 574 subset / 576 baseline / 631 superset → conformance-safe. In-repo labels corrected `62044c5`→`b30a589` (2026-07-12) across the 9 v7.75-re-run peer reports pairing the `576·0F·89S` figure with `62044c5` (common-lisp, csharp, elixir, haskell, java, ocaml, swift, typescript, zig; C/Ada already carried `b30a589`). Any external arch-side scorecard remains arch's to pull. Was THE handoff item (PHASE-S5 §6). |
...
| **A-C-011** raw-pthread concurrency-churn flake | reliability | peer/operator | **OPEN (observed 2026-07-12)** — 1 of 3 `--profile core` runs aborted (SIGABRT, core dumped) at `concurrency.t2_2_connection_churn` cycle ~48; the other 2 were clean 682·0F. A latent race in the raw-pthread connection-churn path (A-C-004 substrate), surfaced while re-gating the multisig fix — NOT caused by it. Gate is clean on a non-flaking run; harden the churn teardown (candidate for a §7b stress-hardening pass). Sibling to A-C-009 (atomic refcount). |
