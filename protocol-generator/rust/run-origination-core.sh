...
if [ "${INCONTAINER:-0}" != "1" ]; then
  ...
  exec podman run $PODMAN_RUN_CAPS --rm --network=none --security-opt label=disable -e INCONTAINER=1 \
    -v "$HOSTREPO":/work:Z -w /work entity-core-keystone/rust-toolchain:latest \
    sh /work/protocol-generator/rust/run-origination-core.sh "$@"
fi
