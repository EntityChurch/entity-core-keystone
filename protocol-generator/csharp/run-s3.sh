#!/usr/bin/env bash
# S3 peer machinery — the peer test project (test/EntityCore.Protocol.Tests).
#
...
podman run $PODMAN_RUN_CAPS --rm --network=none \
  -v "$REPO_ROOT":/work:Z \
  entity-core-keystone/dotnet9:latest \
  bash -lc 'cd /work/protocol-generator/csharp && dotnet test test/EntityCore.Protocol.Tests -p:UseAppHost=false --nologo'
