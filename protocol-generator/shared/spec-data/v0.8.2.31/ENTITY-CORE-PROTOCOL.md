...
**This section is the single normative home of the `content_hash_format` registry `[MUST]` (0.8.2.27).** §8.2's algorithm table and `ENTITY-CBOR-ENCODING.md` §4.3, §4.4 and §4.6 restate this table for their own readers, and each names this section as its authority; **none of them may state it differently**, and an allocation is made here first. The registry owns three things together and they cannot be split across homes: which code denotes which algorithm, each code's validation state, and the `(format_code, digest)` address-space semantics below.

*(0.8.2.27 — `ENTITY-CBOR-ENCODING` §4.3 and §4.4 transposed `0x03` and `0x04` against this table, so the corpus bound the name `"ecfv1-blake3"` to two different codes and a peer advertising it hashed under a different one depending on which document its author read. §4.5 additionally specified the code as a fixed `OCTET`, the form that preceded the v7.66/v7.67 varint reframing — its tell was a `0xFF` "extension mechanism if >254 formats needed", an escape hatch only a one-byte field needs, in a registry whose own rule is that 255 is never allocatable because a `0xFF` byte cannot terminate a varint. Neither restatement named an authority, so the drift was invisible from both ends; the pin above is what makes the next one a grep. Reported as `F20` by `entity-system-conformance` while authoring the `encoding` check batch.)*
...
- Map keys sorted by encoded byte length, then lexicographically — **RFC 8949 §4.2.3 (*Length-First Map Key Ordering*), never RFC 8949 §4.2.1 `[MUST]` (0.8.2.27).** This section is the ordering's normative home; `ENTITY-CBOR-ENCODING.md` §4.1 Rule 2 and §9.1 restate it. RFC 8949 §4.2.1's core deterministic requirement sorts by the *bytewise* lexicographic order of encoded keys with no length precedence, and **this protocol does not use it.** The two coincide for pure text keys — a text head byte is monotonic in the string's length, so bytewise already sorts by length there — and diverge for keys of different CBOR major types. ⚠ **Check which ordering your CBOR library's "canonical" mode implements before relying on it.** The two families are both in the wild: RFC 7049 canonical form sorts length-first and agrees with this rule; RFC 8949 §4.2.1's core deterministic requirement sorts bytewise and does not. An implementation that inherits the wrong one produces different bytes, a different content hash, and a different address for the same entity, with nothing in its own test suite to say so — which is why the ordering is pinned here and why a discriminating vector is required. Python `cbor2`'s `canonical=True` has been measured against a discriminating key pair and is **length-first**, so it conforms; one library measured is not a general claim about libraries. *(0.8.2.27 — every normative home stated length-first and `ENTITY-CBOR-ENCODING`'s conformance appendix cited RFC 8949 §4.2.1 in the one cell a vector author reads, so the corpus named the ordering it forbids as the ordering it requires. Reported as `F22`; the filing seat asked for its reading of the RFC to be checked, and it was correct.)*
...
**Wire-acceptance carve-out.** Implementations MAY decode `peer_id`s presented in non-canonical form (e.g., `hash_type=0x01` SHA-256-form for Ed25519) for backwards compatibility with older implementations. Implementations MUST canonicalize on acceptance: wire form is presentation; storage form is canonical. Before any tree-lookup, cap-state operation, or stored-policy entry, the impl MUST canonicalize: derive `(public_key, key_type)` from the presented form (post-handshake when pubkey is available; via the §3.6 lazy-canonicalize path when at mint time without prior contact); recompute canonical form; use canonical for storage and lookup. Impls SHOULD log non-canonical form acceptance at debug level. Impls MAY refuse non-canonical form at construction or handshake under stricter local policy (deployment choice) — §4.6 step 3 pins what that refusal may and may not be reported as. *(0.8.2.29 — the lazy-canonicalize path was cited as §6.2 and lives at §3.6.)*

**Form observation and the correlation prohibition (normative).** A peer SHOULD publish exactly one canonical-form `peer_id` per identity at any operational moment, and SHOULD NOT switch forms over time within an operational deployment; multi-form-per-identity is the experimental case bounded by the note below. An implementation SHOULD cache observed `peer_id` forms for the **connection lifetime** and MAY age them out aggressively — the cache scope is per-connection, and cross-connection form memory is not required of a conformant peer. **When a peer presents a form not previously seen, an implementation MUST NOT auto-correlate it to forms previously observed from other peers `[MUST]` (0.8.2.29).** It SHOULD treat the arrival as a new route observation. Correlation an operator states — minting a cap pattern with prior-contact context — is fine; correlation an implementation *infers* before the handshake reveals a public key is not, and cannot be sound: before pubkey reveal the forms are one-way digests, so inferring identity across them requires either reversing a hash or maintaining a hidden alias index, and the alias machinery is deliberately out of v1 (see the experimental note below). A peer that guesses here merges two identities on a coincidence and resolves one peer's grants for another. *(0.8.2.29 — these are the parts of the v7.65 "mutability norms" amendment that never reached a specification. Its other norms are explicitly labelled restatements of the canonical-form mandate, of this section's canonicalize-on-acceptance rule and of §3.6's cap-pattern rule, and all three landed in those homes; the amendment's own spec-edit instruction was to add a fourth subsection restating them, which would have created the competing home this corpus removes on sight. What was genuinely unstated is the per-connection cache scope and the correlation prohibition — and the prohibition is a MUST NOT that had been carried in a document headed "six normative SHOULDs", so even where it was read it was read at the wrong level.)*
...
1. **Validate on receipt**: Compute expected hash from `{type, data}`, compare with `content_hash`, reject on mismatch. **This is self-consistency, and it is only half of the obligation `[MUST]` (0.8.2.23).** An entity that hashes to its own content is still wrong under an address that is not its hash — so: **resolution integrity — an implementation MUST NOT resolve an entity used for any authority decision through an address it has not verified against that entity's content.** Two mechanisms satisfy this and an implementation MUST implement at least one: **(a) bind the key** — on accepting a received `included` map, reject any entry whose key is not `content_hash({type, data})` of the entity under it *(RECOMMENDED for new implementations: it fails the envelope closed at one site)*; or **(b) discard the key** — drop wire-supplied keys at the decode boundary and address entities exclusively by a `content_hash` validated under this item. ⚠ **Mechanism (b) depends on this item running at EVERY ingress and MUST NOT be adopted without it**: addressing by `content_hash` is safe only because that field has itself been validated, and a peer that skips the validation pass has re-created the defect one level down, with the address an unvalidated wire field again. **Retaining a wire-supplied key as an address without binding it is non-conformant.** The map's keying is stated as a property of the envelope in §3.1; this item is the obligation on the receiver. Dispositions are enumerated in §5.2a.
   ...
   **Which received entities this item binds `[MUST]` (0.8.2.28).** It binds every entity the peer receives **as an entity**, at the boundary where it is received. The strengths are not uniform because the exposure is not, and assembling the answer previously required four sections:

   | Surface | Strength | Code on failure | Scoreable as a refusal |
   |---|---|---|---|
   | **EXECUTE / EXECUTE_RESPONSE root** | **MUST** (this item; §6.5 routes it) | **400 `hash_mismatch`** | **yes** — and it is a pre-admission refusal, so a bare close and a silent drop are each non-conformant (§4.11) |
   | **`included` entities** | **MUST** (§3.1) — self-consistency **and** this item's resolution integrity | **400 `hash_mismatch`** | **yes**, except an **unreferenced** mis-keyed entry, which has no uniform verdict and MUST NOT be asserted either way (see the paragraph above) |
   | **`params` / `result`** | **SHOULD**, MAY defer (§3.4) | — | **no — witness only**, unless the embedded hash is resolved for an authority decision, where this item's resolution-integrity MUST binds |

   **`params`/`result` are not a weaker instance of this rule; they are outside it**, because they are not separately received entities — they are bytes inside a received entity whose hash this item already validates at MUST, and §3.4 states that reasoning. What reaches back into MUST territory is **use**, not receipt: the moment a handler resolves anything through `params.content_hash` for an authority decision, the resolution-integrity obligation above governs and §3.4's *"MAY defer"* does not reach it.
...
- Accept any byte sequence at the boundary that is not canonical ECF. **The refusal is `400 non_canonical_ecf` (§4.7, §4.11 arm 5a) `[MUST]` (0.8.2.27)**, and it is a pre-admission refusal like any other: a coded frame is mandatory and a silent drop is non-conformant. **Detection is a re-encode-and-COMPARE** (`ENTITY-CBOR-ENCODING.md` §10.3, §5.4 precondition (a)) — a content-hash check alone does not see this, because a sender that encodes non-canonically while hashing the canonical form produces a hash that verifies. *(0.8.2.27 — this clause named no code, and `ENTITY-CBOR-ENCODING` §9.2's "Decoders MUST accept any valid CBOR (not just deterministic)" contradicted it at `MUST` level in the section an implementer reads when writing a decoder. A peer had no conformant way to comply and no code to comply with; `entity-system-conformance` shipped the requirement `disputed` and said so.)*
...
This is a core type used in the protocol for peer identity. All peer ID values — the `peer_id` field on connect `authenticate` entities, on `system/connection` and the `system/peer/status` family, and every peer-id carried in a path segment — use the same string representation. *(0.8.2.29 — this sentence read "`peer_id` on `system/peer` entities and connection messages". `system/peer` carries no `peer_id` field: §3.5 keeps it out of the hashable basis and `data` is the hashable basis. The type is unaffected; only the example holder was wrong.)*
...
| 501 | Not supported. Default `code` = **`unsupported_operation`** — a handler IS registered at the path and does not implement the named operation (§6.2). **`unknown_operation` is a synonym and MUST NOT be emitted** (0.8.2.6). **Reachable only after `check_permission` passes — an unauthorized operation is `403` whether or not it is implemented (§6.5, §6.7; 0.8.2.30).** |
...
**Validation**: Implementations SHOULD validate `params.content_hash` and `result.content_hash` against their `{type, data}` on receipt, consistent with entity fidelity (§1.8). The outer EXECUTE/EXECUTE_RESPONSE content_hash covers the params/result bytes (including the embedded content_hash), so the EXECUTE hash is valid even if the params hash is not independently checked. However, handlers that use `params.content_hash` for sub-requests or storage SHOULD validate it to prevent propagation of incorrect hashes. Implementations MAY defer validation to handler processing time rather than dispatch time. **This SHOULD and this deferral are scoped to handlers that do not resolve through the embedded hash `[MUST]` (0.8.2.28).** The moment a handler resolves anything through `params.content_hash` or `result.content_hash` **for an authority decision**, §1.8 item 1's resolution-integrity obligation governs unconditionally — *an implementation MUST NOT resolve an entity used for any authority decision through an address it has not verified against that entity's content* — and neither the SHOULD nor the deferral above reaches that case. *(0.8.2.28 — this paragraph was read as the whole rule for `params`, which is what a reader looking up the field would do, and the sentence that raises it to MUST lives in §1.8 under a different noun. The two were never in conflict: the SHOULD is about receipt, where the outer hash already covers these bytes, and the MUST is about use as an address.)*
...
**Cap-pattern peer-reference canonicalization.** A `peers:` IdScope value is a **flat peer-id** — a bare Base58 identifier or the wildcard `*`, never a path `[MUST]` (0.8.2.29) — and MUST be canonical form per §1.5. Four normative implementation behaviors:
...
2. **At match-time** — the comparand is the peer under test, `extract_peer(execute.uri, local_peer_id)` (§5.2), which is canonical per the §1.5 mandate plus the wire-acceptance canonicalize-on-storage rule; literal string comparison against stored canonical-form patterns proceeds normally per the id-scope grammar above. *(0.8.2.29 — this rule read "the runtime `peer_id`" of an arriving peer. Since 0.8.2.2 the dimension is evaluated against the **target** peer of the dispatch, not the caller, and on the inbound path §6.5 step 3 has already forced them equal; the two diverge on §1.4's internal-dispatch class, which is the only class where this dimension does work.)*
3. **At mint without pubkey (lazy canonicalize)** — when an operator mints a cap pattern using a non-canonical Base58 handle for a peer the impl has not previously contacted, an impl that does not refuse under §1.5's stricter-local-policy carve-out MUST accept the mint with the non-canonical form, recording the pattern as `pending-canonicalization`. On first contact with the named peer (handshake reveals pubkey), the impl MUST canonicalize the pending pattern in place and emit a debug log. Subsequent matches against the canonicalized pattern follow rule 2. *(0.8.2.29 — this read "the impl MUST accept the mint", which contradicts §1.5's "MAY refuse non-canonical form at construction … under stricter local policy" at MUST level and made every strict deployment non-conformant. The obligation is on an implementation that has chosen to accept; the carve-out is the prior question.)*
4. **On a RECEIVED pattern, canonicalize at COMPARISON `[MUST]` (0.8.2.29)** — a capability arriving over the wire is signed, so its stored bytes cannot be rewritten without destroying the signature; rules 1 and 3 are mint-side and do not reach it. A verifier MUST therefore canonicalize the pattern value and the comparand into §1.5 canonical form *for the comparison* and MUST NOT compare a received non-canonical literal directly. **Where the value cannot be canonicalized** — a SHA-256-form handle for a peer never contacted, so no pubkey is available — the dispositions are **not symmetric**: an `include` entry that cannot be canonicalized **MUST NOT match** (the grant is withheld), and an `exclude` entry that cannot be canonicalized **MUST match** (the peer is excluded). Both fail closed. *(0.8.2.29 — a received non-canonical `peers.exclude` had no verifier rule at all, and the literal matcher the id-scope grammar mandates therefore silently failed to match it: the excluded peer was not excluded, which is authority granted by a canonicalization gap. The include direction fails safe under the same matcher and the exclude direction inverts, which is the scope-position asymmetry already ruled for the path-scope sentinel at 0.8.2.21, arriving on the id-scope surface.)*

**The two canonicalizations are orthogonal and conflating them is the defect `[MUST]` (0.8.2.29).** §1.5 **form** canonicalization — one wire-form of a peer-id normalized to the canonical form of the same identity — applies to every peer-id value including these, and is what rules 1, 3 and 4 perform. §5.4 **path** canonicalization — leading-`/` universal scope, interior peer-wildcards, peer-relative qualification — is forbidden on id-scope by the grammar above and is the F40 defect. An implementation that skips the first because the grammar forbade the second has the `exclude` fail-open in rule 4; one that applies the second because the first was required has the F40 over-grant. **Neither is a reading of the other.**
...
*(0.8.2.29 — this block's header sentence wrote a `peers:` IdScope value as `"/{peer_id}/path"`. That form predates the id-scope grammar above, which matches id-scope values as literal strings with no path transforms: a path-shaped `peers:` pattern therefore matches **no** bare peer-id value at all, silently, so an `include` written this way grants nothing and an `exclude` written this way excludes nobody. The §3.6 field table and §5.2's reachability rule both already described flat values; this sentence and §6.2's note were the surviving pre-F40 shape.)*
...
Conformance: peers that don't populate `system/peer/self` are still core-conformant. They lose tree-walkability for the local-peer entity; handlers that need the entity's content_hash can still compute it, because the entity content is `{public_key, key_type}` (§3.5) and a peer always holds its own key material.

Foreign-peer `system/peer` entities are NOT bound at fixed tree paths. They are content-addressed and reached via `envelope.included` (§6.5) or by content_hash lookup in the local content store. The `system/peer` entity content is `{public_key, key_type}` (§3.5), which is recoverable from an **identity-form** peer-id directly and from a SHA-256-form one only after prior contact (§3.5's derivation note) — so for the identity-form case binding foreign entities at tree paths adds no information beyond what the operational-state subtrees (`system/peer/status/`, `system/peer/transport/`) already enumerate, and for the SHA-256-form case the `included` map or the content store is the only route. *(0.8.2.29 — both sentences read "the entity content is `{peer_id}`, deterministic from peer_id", a third declared shape for this entity alongside §3.5's `{public_key, key_type}` and the type system's retired three-field form. It was also the one shape no section ever specified, and it made a derivation that holds only for identity-form peer-ids read as unconditional.)*
...
| `system/peer` | Entity type — `{public_key, key_type}` keypair-bearing entity (§3.5); `peer_id` is **not** a field (0.8.2.29) |
...
- ⛔ **`ping` is an EXAMPLE here and is NOT a core operation `[MUST]` (0.8.2.30).** This document
  defines no `ping` operation: no params type, no result type, no manifest entry and no §9 floor row.
  It names `ping` only to illustrate the keepalive class, and **`EXTENSION-NETWORK` is its owner and
  its only definition.** The sentence above is therefore **conditional on implementing it** — a peer
  that implements `ping` on the connect path MUST serve it without `author`, `capability` or
  signature; **a core-profile peer that does not implement it is conformant.** A peer that does not
  implement it answers **`501 unsupported_operation`** (§6.2): the connect handler IS registered at
  this path and the named operation is not implemented, which is that row exactly. **`400
  invalid_request` is NOT conformant for this input** — it says the request was malformed, and a
  well-formed call to an unimplemented operation is not. *(0.8.2.30 — the illustrative `ping` was
  read as a core requirement, which is the only reading under which a peer that omits it is
  non-conformant. Independent implementations diverged on the code for this input while agreeing on
  the behaviour — refusing correctly and naming the refusal wrongly, which is the same synonym class
  as §6.2's `unknown_operation`. A peer that implements the operation and validates its params is
  not the outlier it looks like: serving the call without author or capability is what the rule
  above obliges.)*
...
; Authenticate EXECUTE_RESPONSE envelope
{
  ...
  "included": {
    ...
    h'00...': {                                ; Granter's peer entity
      "type": "system/peer",
      "data": {                                ; peer_id is NOT a field (§3.5)
        "public_key": h'...',
        "key_type": "ed25519"
      },
      "content_hash": h'00...'
...
**The `system/peer` entry above carries two fields and not three `[MUST]` (0.8.2.29).** §3.5 is the normative home: `peer_id` is not in this entity's hashable basis, and because a content hash covers the whole canonical encoding of `{type, data}`, it is therefore not a field. The example matters more than most: it is labelled normative, it is the shape an implementer copies, and the value it produces is `signature.signer`, a capability's `granter`/`grantee` and the `{peer_id_hex}` path segment all at once — so a third field here yields a second `content_hash` for one identity and fails §5.2's `signer == author` equality at connect, on a correct signature with a correct key. *(0.8.2.29 — this block carried the pre-v7.65 three-field shape. `0.8.2.15` corrected §4.5a item 1a and §4.6's `peer_entity` pseudocode for the same rule and stopped there.)*
...
1. Every entity a peer authors **on the wire/identity surface** for transmission on this connection — the EXECUTE and EXECUTE_RESPONSE envelope framing, the capabilities it mints for this connection, and signatures — MUST be content-hashed under the active format. This is the surface where identity-equality (`grantee == author`, `signer == author`; §5.2) is evaluated; keeping it single-format per connection is what makes that equality byte-exact.
   ...
   **The exception reaches item 2 and §4.5's transmission prohibition as well `[MUST]` (0.8.2.29).** Item 2 and §4.5 forbid transmitting an entity whose `content_hash_format` lies outside the connection's negotiated active value. A floor-authored `system/peer` entity on a non-floor connection is exactly that shape, and the handshake requires sending it — so read without this sentence the pin makes every non-floor connection unestablishable. The identity entity is exempt from the transmission prohibition **on every connection**, and it is the only entity that is. Nothing else in `included` gains an exemption.

   **"Every consumer derives its hash" is a property, NOT a prohibition on fetching `[MUST]` (0.8.2.29).** The clause states why this entity is `[derive-to-meet]` — its content is recoverable, so pinning its format costs nothing — and it does **not** forbid obtaining the entity from `envelope.included` or from the content store. §5.5's verification algorithm resolves `included[current.data.grantee]` and is conformant; §3.5's pre-v7.65 chains remain verifiable as that section says. A verifier that refuses to look up an identity entity because item 1a said *derives* would reject every legacy chain and every conformant envelope that carries one — which the clause neither says nor implies. *(0.8.2.29 — the sentence is rule-shaped and sits in a normative numbered item, so it read as an obligation, and one rule-shaped sentence in this item had already produced a divergence at 0.8.2.15. What it constrains is the FORMAT the value is derived under, never the ROUTE by which the entity is obtained.)*
...
3. **Identity binding.** The responder MUST verify that `authenticate.peer_id` is the peer-id derived from `authenticate.public_key` (per the §1.5 / §7.4 construction `Base58(key_type ‖ hash_type ‖ hash(public_key))`). A mismatch MUST be rejected with status **401 `identity_mismatch`**. (Without this, step 2 proves possession of *some* private key but not the key for the identity the caller claims — grants resolve by `remote_peer_id`, so an unbound proof lets a caller act under another peer's identity.)
   ...
   **This step tests the BINDING, not the FORM `[MUST]` (0.8.2.29).** The derivation is run **in the `hash_type` the presented `peer_id` declares**, not in the canonical one: a peer-id in a non-canonical but well-formed wire form (§1.5's wire-acceptance carve-out — e.g. SHA-256-form for Ed25519) that decodes to the presented `public_key` **binds correctly and MUST NOT be refused `401 identity_mismatch`**, because the identity it claims *is* the identity it proves. `identity_mismatch` is reserved for a peer-id that resolves to different key material, which is the attack this step exists to stop; spending it on a presentation form tells an honest caller their key does not match itself. The responder MUST then **canonicalize on acceptance** per §1.5 — the stored `remote_peer_id`, every tree lookup and every cap-state operation use the canonical form, never the presented one.

   **A deployment that refuses the non-canonical form refuses it as a form `[MUST]` (0.8.2.29).** §1.5 permits refusing at the handshake under stricter local policy; that refusal is a connect-auth pre-admission refusal (§4.11) and its code is **`400 unsupported_key_type`** where the form is one this peer does not implement, and otherwise **`401 identity_mismatch` is NOT conformant** — a strict peer emits **`400 invalid_request`**, whose remedy (*re-present in canonical form*) is the one the caller can act on. **Whether a peer is strict is local policy and is NOT a conformance surface**: a check set MUST NOT assert that a peer accepts the non-canonical form and MUST NOT assert that it refuses one, and MUST accept either outcome on that input — but where a peer accepts, the canonicalize-on-acceptance obligation above is drivable and is asserted. *(0.8.2.29 — §4.6 step 3 and §1.5's carve-out gave opposite outcomes on one input, both keyworded: the step read as though any non-canonical presentation were a mismatch, and the carve-out reads as though acceptance were the default. Two conformant implementations therefore answered `401` and `200` to the same hello. The reconciliation is that they are about different properties — the step is about which key, the carve-out is about which spelling — and the code for the second was never allocated, so a strict deployment had no conformant way to say no.)*
...
| **Non-canonical ECF — bytes that DO decode as CBOR and are not canonical** (0.8.2.27; §1.11 boundary, `ENTITY-CBOR-ENCODING.md` §9.2) | `non_canonical_ecf` | 400 |

The responder MUST emit the coded error EXECUTE_RESPONSE before closing the connection on any of the above (§4.6 status boundary).

**`non_canonical_ecf` names a CLASS, and the tag-policy rule is one member of it `[MUST]` (0.8.2.27).** §1.11 forbids admitting any byte sequence at the boundary that is not canonical ECF. Every input in that class takes the **same remedy** — *re-encode canonically* — which is why it takes one code, and why minting a second for any member would be the synonym this table forbids. Members include a CBOR major-type-6 item in a data-field position (`ENTITY-CBOR-ENCODING.md` §6.3, which is the detection rule for that member and not the definition of the code), an indefinite-length item, a non-minimal integer encoding, and map keys out of §1.3 order.

**The tag member's scope is ANY NESTING DEPTH, and `ENTITY-CBOR-ENCODING.md` §6.3 is its sole authority `[MUST]` (0.8.2.29).** *"In a data-field position"* is §6.3's own shorthand and §6.3's next sentence defines it: **any** major-type-6 item appearing anywhere within an entity's `data` field — top level of `data`, inside nested arrays or maps at any depth, and inside the `data` of any `included` entity. **Tag 55799 is a member like any other**: §6.3 permits it as a file-format marker and forbids it in wire protocol messages, so a frame wrapping a whole `data` value in it is refused. No restatement of this member anywhere may narrow that scope by carrying the shorthand without the definition; where a restatement and §6.3 differ, §6.3 governs. *(0.8.2.29 — every restatement of this member in this document carried the four-word shorthand and none carried the scope, so the narrow reading — a tag only at the top level of `data` — was available to an implementer who read the enumeration rather than the detection rule. All three ground-up implementations drive the wide reading, including tag 55799 and depth-nested tags, so the narrow reading described no implementation. This is the same shape as `0.8.2.28`'s missing enumeration row, inverted: there a home omitted a member, here the homes carried the member and dropped its bound.)*

⛔ **The two exclusions ruled in 0.8.2.24 and 0.8.2.26 stand unchanged and are not reopened.** This code is **NOT** conformant on §4.11's **framing arm** — bytes that do not decode into CBOR at all are `400 invalid_request`, because *"your bytes are truncated"* is not *"re-encode."* It is **NOT** conformant for a **mis-keyed `included` entry** (§5.2a) — that encoding is canonical and what is false is the claim the key makes, so it is `400 hash_mismatch`. The three codes partition the input by the remedy each selects: malformed frame → `invalid_request`; decodes but not canonical → `non_canonical_ecf`; canonical but its claim about the content is false → `hash_mismatch`.

*(0.8.2.27 — this code had **no row in this table at all**, while living in `ENTITY-CBOR-ENCODING` §6.3 and in §4.11's cause table, and this table declares itself the normative MUST-emit contract and the one an implementer reads when the input arrives. The gap was found answering `F21`, which asked what a peer refuses non-canonical bytes **with** and could get no answer from the corpus. Worse, it was ours by one word: 0.8.2.24 and 0.8.2.26 correctly ruled the code off the framing and mis-keyed-`included` arms, and wrote that scope as "§6.3 defines that code for tag-policy violations **specifically**" — narrower than the remedy argument those revisions were themselves built on, and narrow enough to leave the class the code is named for with no code. The exclusions were the ruling; "specifically" was the instrument, and it excluded more than the argument did.)* `invalid_signature` (the pre-v7.61 spelling for the step-2 failure) is superseded by `authentication_failed`; impls emitting `invalid_signature` for a connect-auth signature failure should migrate.
...
- **A frame that never becomes an Envelope at all `[MUST]` (0.8.2.25; scoped 0.8.2.26)** — un-parseable bytes, a truncated frame, or a length prefix that never completes — and a frame whose root entity is neither EXECUTE nor EXECUTE_RESPONSE (§3.3). These are **pre-admission refusals** and §4.11 governs their emission. **Bytes that DO decode as CBOR but are not canonical ECF are not this arm**: that is the `400 non_canonical_ecf` refusal rowed above, a pre-admission refusal under its own row of §4.11's table (0.8.2.26; scope corrected to the class 0.8.2.27). A CBOR tag in a data-field position (`ENTITY-CBOR-ENCODING.md` §6.3) is the named member of that class, not its boundary.
...
**The half-open connection is an out-of-order state, and it is named here because neither neighbouring rule reaches it (0.8.2.8).** A connection that has completed `hello` but not `authenticate` is **half-open**: it is *not* established. The out-of-order row above governs it — a connect operation the responder implements, arriving in a state that forbids it — so an unauthenticated `ping` on a half-open connection is **409 `connection_sequence_error`**. This is stated because two adjacent rules each *look* like they cover it and neither does: the `invalid_nonce` row is scoped to a pre-**hello** `authenticate` (§4.6 step 1), and the pre-authorized-connect exception **does apply here but decides a different question** — it exempts the path from carrying `author` and `capability`, in **any** connection state (§4.2), and it says nothing about which connect operations are legal in which state. **Being pre-authorized is not being in-order.** So the `ping` is admitted without credentials and is then refused as out-of-order: **409**. All three ground-up implementations answer 409 correctly by construction; the row is made explicit so the behaviour is pinned by text rather than by three independent derivations. *(0.8.2.30 — this passage read "the pre-authorized-connect exception … is scoped to an **established** connection, which a half-open one is not", which contradicts §4.2's "in any connection state — before and after establishment" at the same level. The 409 outcome was right and its stated premise was not, and the premise is the half a reader carries to the next case: read literally it re-introduces exactly the state qualifier `0.8.2.6` removed, and would make a post-establishment unauthenticated `ping` the only exempt input.)*
...
**And where the close is a choice, it is bounded by what it costs `[MUST]` (0.8.2.29).** The choice above is a choice about the *connection*, never a licence to end requests the peer has already admitted on it: where **any admitted request is in flight on that connection**, §4.9(c) owes each of them a response and §4.10 forbids degrading service to them, so the peer **MUST NOT** close and MUST continue after emitting the coded frame. This binds every whole-decoded member of this class identically — a forbidden tag, a root-entity-hash failure, a mis-keyed `included` entry — because the property that makes continuation possible is the same one in each case: the length prefix completed, so the stream is synchronized on the next frame boundary and the cause of the refusal says nothing about the peer's ability to read the next frame. **Continuing is therefore the only disposition that is conformant on every connection**, and a peer that closes must first establish that nothing admitted is outstanding. *(0.8.2.29 — the choice sentence was unqualified and the constraint on it lived in this section's Conformance paragraph, which states the multiplexed arm and is four paragraphs away; neither referenced the other. An implementation reading the choice sentence alone can close on a multiplexed connection and believe itself conformant, and one implementation reached three different dispositions for three members of one class — continue on an undecodable frame, continue on a forbidden tag, close on a hash failure — with only the third derived from a rule that does not distinguish them.)*
...
| **Root entity self-consistency — the root's own `content_hash` is not `content_hash({type, data})` of the root** (0.8.2.28) | **400** `hash_mismatch` | §1.8 item 1, §6.5 |
...
| **Non-canonical ECF — bytes that DO decode as CBOR and are not canonical.** Members include a CBOR major-type-6 item **anywhere within a `data` field at any nesting depth** (§6.3), an indefinite length, a non-minimal integer, and map keys out of §1.3 order | **400** `non_canonical_ecf` | §4.7; §1.11; `ENTITY-CBOR-ENCODING.md` §9.2, §6.3 |
...
**`400 non_canonical_ecf` is NOT conformant on the framing arm `[MUST]`, and the framing arm is bytes that DO NOT DECODE `[MUST]` (0.8.2.26).** §4.7 defines that code for **non-canonical ECF** — bytes that decode as CBOR and are not canonical, of which a major-type-6 item in a data-field position (`ENTITY-CBOR-ENCODING.md` §6.3) is one member — and *"your bytes are truncated"* is not *"re-encode."* *(Scope corrected 0.8.2.27: this sentence read "§6.3 defines that code for CBOR tag-policy violations **specifically**", which is narrower than the remedy argument it rests on and left the rest of the class with no code at all. The exclusion this paragraph rules — not the framing arm — is unchanged.)* This is §5.2a's ruling for the adjacent arm, generalized: **the code selects the caller's remedy**, so a code that is merely in the right family is still wrong. **The two rules partition the input and do not overlap:** bytes that do not decode into CBOR at all are this section's framing arm, `400 invalid_request`; bytes that decode and carry a forbidden tag are §6.3's, `400 non_canonical_ecf`, and that refusal is a pre-admission refusal governed by this section's frame obligation like any other member. *(0.8.2.26 — this row read "un-parseable, truncated or **non-canonical** CBOR", and a CBOR tag on a data field satisfies that row and §6.3's MUST simultaneously and incompatibly: it is non-canonical, it is detected at decode time, and it never becomes an Envelope. The row's own justification — the code selects the remedy — is correct for the truncated sub-case and was never true of the tag-policy one, where *re-encode without the tag* is precisely the remedy and §6.3 is the section that says so. The word "non-canonical" reached a rule the argument beneath it did not.)*
...
resolve_peer_scope(peers_scope, local_peer_id):
  ; 0.8.2.29. §1.5 FORM canonicalization for the `peers` dimension, whose
  ; values are peer-ids -- the ONLY id-scope dimension of which that is true
  ; (`operations` values are operation names and this does not apply to them).
  ; Orthogonal to the §5.4 PATH canonicalization the id-scope arm of
  ; scope_value_matches forbids: skipping this one because the grammar forbids
  ; that one is the exclude fail-open this function exists to close (§3.6).
  ;
  ; A received capability is SIGNED, so its stored bytes cannot be rewritten --
  ; the mint-side rules (§3.6 rules 1 and 3) do not reach it. Canonicalization
  ; therefore happens HERE, for the comparison only.
  ;
  ; canonical_peer_id(p) returns null when p cannot be resolved: a SHA-256-form
  ; handle for a peer never contacted, so no public key is on hand. The
  ; disposition of an unresolvable value DEPENDS ON ITS POSITION and the two
  ; directions are not symmetric -- the same asymmetry, and the same reason,
  ; as the unmatchable-exclude arm of matches_scope above.
  include_out = []
  for p in peers_scope.include:
    if p == "*": include_out.append(p); continue
    c = canonical_peer_id(p, local_peer_id)
    ; UNRESOLVABLE INCLUDE -> DROP IT. The grant is narrower than its author
    ; wrote, which is the fail-CLOSED direction: authority is withheld.
    if c is not null: include_out.append(c)
  if include_out is empty: return null
  exclude_out = []
  for p in (peers_scope.exclude or []):
    if p == "*": exclude_out.append(p); continue
    c = canonical_peer_id(p, local_peer_id)
    ; UNRESOLVABLE EXCLUDE -> REFUSE THE GRANT. We cannot tell whether the
    ; value under test IS the peer this entry names, and treating "cannot
    ; tell" as "does not match" carves out nothing: the grant is SILENTLY
    ; WIDER than its author wrote and the excluded peer is admitted. That is
    ; authority granted by a canonicalization gap. Returning null here makes
    ; the caller skip this grant; another grant may still authorize.
    if c is null: return null
    exclude_out.append(c)
  return {include: include_out, exclude: exclude_out}
...
check_permission(execute, capability, handler_pattern, local_peer_id):
  ...
  for grant in capability.data.grants:
    ...
    if not matches_scope(handler_pattern, grant.handlers, "system/capability/path-scope", local_peer_id):
      continue
    ; 0.8.2.29: `peers` is the one id-scope dimension whose values are peer-ids,
    ; so §1.5 FORM canonicalization applies to BOTH operands before the literal
    ; match. This is NOT the §5.4 PATH canonicalization the id-scope arm forbids
    ; -- the two are orthogonal and conflating them is the defect (§3.6).
    ; Returns null when an `exclude` entry cannot be resolved: fail-closed, the
    ; same safety direction the unmatchable-exclude arm takes for path-scope.
    peers_scope = resolve_peer_scope(grant.peers or {include: [local_peer_id]},
                                     local_peer_id)
    if peers_scope is null:
      continue
...
- **`501 unsupported_operation`** — a handler IS registered at the path, but does not implement the named operation. The caller's authority is irrelevant *to which code this failure carries*; the operation does not exist on this handler. **This sentence is general and is not scoped to the capability handler (0.8.2.6).** It states the rule for *any* handler; §3.3's 501 row is the authority and this is its restatement at the site that motivated it.

   ⛔ **It pins the CODE and does NOT order the check `[MUST]` (0.8.2.30).** *"The caller's authority is irrelevant"* means **do not report this failure as `403`**; it does not mean *test operation-existence before authority*. **`501` is reachable only after `check_permission` has passed** — §6.5's dispatch chain places *Handler processes operation* after *Check permission*, and §6.7 states the consequence outright. ⇒ **A request that is both unauthorized and unimplemented answers `403`, not `501`**, and this row is never the one that reports it. *(0.8.2.30 — the sentence was generalized at `0.8.2.6` to fix two implementations emitting `400 unknown_operation`, which is a CODE defect, and the generalization was then read as an ordering claim. An implementation on the ordering reading answers `501` to an operation the caller's grant does not cover, which lets an unauthorized caller **enumerate a handler's operation set** one probe at a time — the operation-existence leak §6.7 names and refutes, and refutes **on the strength of this ordering**. A fold that pins a code does not license inferring an order from its neighbourhood.)* **`unknown_operation` is a minted synonym and MUST NOT be emitted** — on the `incompatible_key_type` / `invalid_signature` precedent. (The heading above scopes this subsection to the capability handler, and the scope was read onto the rule: two implementations emit `400 unknown_operation` from every non-capability handler — a different status *and* a different code for the failure this line governs.)
...
**Namespace disambiguation (0.8.1, UN-a/F47; narrowed 0.8.1, CAP-7; basis corrected 0.8.2.29).** This policy-path `{peer_pattern}` — a single path segment, closed to the three forms above — is a **distinct namespace** from the capability **`peers:` scope** IdScope patterns (§3.6, §5.2). They collide only in the informal name "peer pattern," and they are closed to *different* sets of forms: this policy path admits the three forms above, while a `peers:` value is a flat canonical peer-id or the bare wildcard `*`. **What MUST NOT be accepted at this policy path is any value carrying a `/`** — a scope pattern is not a policy key, and neither is a path. The partial-prefix closure below (no `00abc*`, in **either** encoding) applies here and MUST NOT be applied to the `peers:` scope patterns; the two namespaces are individually correct. *(0.8.2.29 — this note stated the prohibition as "the path-shaped `peers:` scope form (`/{peer_id}/…`, with slashes)" and illustrated `peers:` with that form. §3.6 now pins `peers:` values as flat, so a prohibition worded against a form the cited dimension cannot take would have protected nothing while reading as though it did. The slash exclusion is what was load-bearing and it is stated directly.)*
...
Frame received
  ...
  |
  +- Reject bytes that are not canonical ECF
  |   (the bytes DID decode; this is encoding, not framing — 0.8.2.26)
  |   ; the class, not one member (0.8.2.27): a CBOR tag in a data-field
  |   ; position, an indefinite length, a non-minimal integer, or map keys
  |   ; out of §1.3 order. Detect by re-encoding to ECF and COMPARING
  |   ; (§5.4 precondition (a)) — a hash check alone does not see it.
  |   → 400 non_canonical_ecf, coded frame; MAY then close
  |     (§4.7, §1.11; ENTITY-CBOR-ENCODING §9.2, §6.3; §4.11)
  |
...
**Operation-existence is tested INSIDE the handler step, after `check_permission` `[MUST]` (0.8.2.30).** The chain above places *Handler processes operation* after *Check permission*, and that order is normative rather than illustrative. An implementation MUST NOT answer **`501 unsupported_operation`** to a request whose operation `check_permission` would refuse: the `operations` dimension is one of the four that step evaluates, so a request naming an operation outside the caller's grant is **`403`**, whether or not the handler implements it. `501` is reached only where permission **passed** and the resolved handler does not implement the named operation.

**This is a confidentiality property and not a preference.** Testing operation-existence first makes the response a two-valued oracle over the handler's manifest for a caller holding *any* grant on that path: probe an operation, read `501` against `403`, and the handler's whole operation set is enumerable without authorization for it. §6.7's masking discussion names exactly this expansion and dismisses it — *"the tempting operation-existence-via-`501` expansion is refuted: `501` is emitted post `check_permission`, so it leaks nothing"* — so the leak is closed **by this ordering and by nothing else.** A peer that reverses the two re-opens it while every other rule still reads as satisfied. *(0.8.2.30 — the order was drawn in the chain and asserted in §6.7's parenthesis, and §6.2's code pin was independently readable as an ordering claim. An implementation was measured answering `501` on a grant covering one operation of the targeted handler, which is the leak, reached from a sentence about codes. Neither of the two homes that had the order right stated it as an obligation.)*
...
> **A row here that restates a rule stated elsewhere NAMES that section as its normative home `[MUST]` (0.8.2.31).** Many rows below are restatements — the floor is a list an implementer builds from, so it carries the rule in short form — and **a restatement and its authority are two artifacts with one meaning.** When the authority moves and the row does not, the row keeps publishing the superseded rule **positively**, in the one document a peer is built to. This has now happened three times: the §1.4 PD-2 row drifted twice in two revisions, and the §6.8 authority-selection row published a discriminator §6.8 had corrected **eight revisions earlier**, found only when an outside seat asked whether the floor had followed a security fix at all. **Naming the home is what makes the pair machine-watchable**; a row that restates without saying so is invisible to every instrument there is.
>
> ⚠ **This is a rule about the ROW, not about the reader.** On any disagreement the named section governs and **the row is the defect** — so a fold that changes a rule edits every row naming it, in the same commit.
...
- **An unmatchable pattern in an `exclude` array excludes EVERYTHING, and a capability carrying one is INVALID (0.8.2.21; the bound added 0.8.2.31).** **§5.4 is the normative home and this row is a restatement**; where they differ, §5.4 governs. The sentinel is fail-closed in an `include` and fail-**open** in an `exclude`; both the refusal at authoring and the deny at evaluation are required, because a single layer author input can make vacuous is not a gate. ⛔ **And the deny is ORDERED (0.8.2.22): in `check_resource_scope`, in BOTH target arms, the unmatchable-exclude arm MUST be evaluated BEFORE the arm's own overlap or match test.** *(0.8.2.31 — this row carried the rule and not its bound, which is the half a peer did not already have: the pattern arm's coverage test is correct in isolation and was **unreachable**, because `patterns_overlap` `continue`s on a sentinel and skips it. A row that states a rule whose reachable form is a later revision's ordering states the part that was never the defect.)*
...
- **The handler-level check's authority is selected by WHETHER THE ACCESS SERVES A LIVE CALLER'S REQUEST (0.8.2.22; this row corrected 0.8.2.31).** **§6.8 is the normative home and this row is a restatement, not a second statement of the rule**; where they differ, §6.8 governs. In service of the caller's request — a path the caller **named**, *or one the handler **derived** within that request* (a listing entry, an extract or snapshot binding, a merge expansion, a subscription payload) — → **the caller's verified capability AND the executing handler's own grant, and BOTH MUST pass**. On the handler's own behalf, serving no caller request (an autonomous write, a continuation's onward leg, the handler's own managed namespace) → **the executing handler's own grant, and only that**. A peer-root dispatch → **no check**. **The propagated `caller_capability` is attribution and is never a dispatch gate.** *(0.8.2.31 — this row read "selected by WHO NAMED THE PATH … handler-derived → the executing handler's own grant … never the authority for a derived path". That is the discriminator §6.8 corrected at 0.8.2.22, which now says "never by who derived the path" and "derivation is not the discriminator" in those words, so the floor row published the superseded rule positively for eight revisions in the list an implementer builds from. A peer built to it flattens the intersection §6.8 requires — making the §6.3 listing filter vacuous and letting a narrow caller merge anywhere the tree handler can reach, which is the hole 0.8.2.22 closed. The fold that closed it enumerated its homes as §6.3's three sites and §6.8's table, under the heading "three homes, one document, two answers"; there were four, and the fourth is this list.)*
...
- **Resolution integrity on any entity used for an authority decision (0.8.2.23; rowed 0.8.2.31).** **§1.8 item 1 is the normative home and this row is a restatement**; where they differ, §1.8 governs. Validating an entity against its own `content_hash` is **half** the obligation: an implementation MUST NOT resolve an entity used for any authority decision through an address it has not verified against that entity's content, and MUST implement **at least one** of §1.8's two mechanisms — **(a) bind the key** on accepting an `included` map, or **(b) discard wire-supplied keys at the decode boundary** and address exclusively by a validated `content_hash`, the latter only where item 1 runs at **every** ingress. ⛔ **This row does not vote a mechanism and a check set MUST NOT either.** The two fail at different points, and §1.8's `0.8.2.26` clause forbids asserting any verdict on an **unreferenced** mis-keyed entry: mechanism (a) refuses it, mechanism (b) has no entry to reach, and the difference is a property of the mechanism rather than of the peer's correctness. The scoreable surface is the disposition row below, not the detection point
- **`included` keying is normative in BOTH directions (0.8.2.23; rowed 0.8.2.31).** **§3.1 is the normative home and this row is a restatement.** The receiver half is the per-entity validation row above. **The sender half is separately drivable and is this row: a sender MUST key each `included` entry by the content hash of the entity it holds.** A peer that emits a mis-keyed envelope is non-conformant whatever its own receive path does
- **The three resolution-integrity dispositions (§5.2a — 0.8.2.23; rowed 0.8.2.31).** **§5.2a's enumeration is the normative home and this row is a restatement.** An **author** resolved through an unverified address is **401 `authentication_failed`**; a **capability** so resolved is **403 `capability_denied`**; a **chain granter or link signer** so resolved is **403 `capability_denied`**. ⚠ **This is an assignment of class, not a requirement to detect at a particular point** — §5.2a's own `[MUST]` is that a uniform verdict across the three is mechanism-shaped and MUST NOT be required, because a key-discarding peer never detects the corruption at all: its lookup misses, and a miss inherits the row of the step that missed
- **The handler frame: `handler_pattern` names the handler that OWNS the operation, and it is REQUIRED and fail-closed (§6.3 — 0.8.2.23; rowed 0.8.2.31).** **§6.3's frame rule is the normative home and this row is a restatement.** The frame answers *whose authority is being spent*, so a handler authorizing an access whose operation belongs to another handler's surface passes **that** handler's pattern, never its own. **An absent, null or empty `handler_pattern` MUST NOT be treated as *match all handlers*** — the parameter has no permissive default, and a call site that cannot name its frame is a defect at that call site. Where an authority genuinely grants handler access with no path component, §5.2's `{include: []}` construction states that **at the grant**, where it is auditable
