...
**Snapshot type:** verbatim copy of the authoritative normative spec files — **no paraphrase, byte-for-byte** (S2). Verified with `cmp` against the source tree at the pinned commit, not by hash alone.
...
| Source path | `specs/` |
| Source git commit | `106834cc5bde47884046d5a5088d629f44ae0c5f` (`106834c`) — resolvable, tree clean at read |
| Read at | 2026-08-21 |
...
|---|---|---|
| `ENTITY-CORE-PROTOCOL.md` | **0.8.2** | `4be521f7dc3133495c33bdf87395b1e2b2e7ee6df6ca7952a122ab0162b7bbdc` |
...
All three recomputed at `106834c` in this session and matched against arch's independently-computed table in `-l` §1 / `-m` §1. Verify integrity: `sha256sum -c` against this table, or diff against the source repo at the pinned commit.
...
|---|---|---|---|
| `ENTITY-CORE-PROTOCOL.md` | changed (`ff8e7666…` → `4be521f7…`) | **197** | 0.8.0 → **0.8.2** |
