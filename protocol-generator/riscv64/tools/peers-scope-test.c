 ...
 * plain, target-agnostic C, so no riscv64-specific changes were needed beyond the build/run
 * path (cross `riscv64-linux-gnu-gcc` + `qemu-riscv64-static`, matching `make host`).
 *
