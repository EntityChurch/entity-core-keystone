#!/bin/sh
# S4 conformance harness — entity-core-protocol-riscv64 (hand-written riscv64 asm peer,
# run under qemu-user). Mirrors asm-arm64/run-s4.sh; the peer is a riscv64 ELF launched via
# `qemu-riscv64-static` while the Go validate-peer oracle is a native x86-64 fedora:43 ELF
# running directly — they share one loopback, sealed with --network=none.
#
# Runs entirely inside the riscv64-toolchain container. Cross-builds the codec .so + the peer
...
#     -v "$PWD":/work:Z entity-core-keystone/riscv64-toolchain:latest \
#     sh /work/protocol-generator/riscv64/run-s4.sh [validate-peer-args...]
#
...
[ -x "$ORACLE" ] || { echo "run-s4: ERROR conformance oracle not found at $ORACLE" >&2
  ...
  exit 3; }
PROJ=/work/protocol-generator/riscv64
CODEC_DIR="${CODEC_DIR:-/work/ffi-generator/c-abi/entity-core-codec-ffi-c/build-riscv64}"
SYSROOT="${RISCV_SYSROOT:-${SYSROOT:-/opt/riscv64-sysroot}}"
cd "$PROJ"

if [ "${NOBUILD:-0}" != "1" ]; then
  make host >/dev/null           # depends on `codec` → cross-builds the riscv64 .so if absent
fi
...
QEMU_LD_PREFIX="$SYSROOT" LD_LIBRARY_PATH="$CODEC_DIR" \
  qemu-riscv64-static ./bin/host $HOST_ARGS >/tmp/host.out 2>/tmp/host.err &
HOST_PID=$!
