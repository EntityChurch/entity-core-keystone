#!/usr/bin/env bash
# run-s2.sh — S2 (codec / crypto-agility) gate for the riscv64 peer.
#
...
#   ./run-s2.sh
# It relaunches itself inside the riscv64-toolchain container. INCONTAINER=1 skips
...
if [ "${INCONTAINER:-0}" != "1" ]; then
  ...
  exec podman run $PODMAN_RUN_CAPS --rm --network=none \
    -e INCONTAINER=1 \
    -v "$HOSTREPO":/work:Z -w /work/protocol-generator/riscv64 \
    entity-core-keystone/riscv64-toolchain:latest \
    bash /work/protocol-generator/riscv64/run-s2.sh "$@"
fi

cd "$(dirname "${BASH_SOURCE[0]}")"

echo "=============================================================="
echo " S2 codec gate — entity-core-protocol-riscv64"
echo "=============================================================="
