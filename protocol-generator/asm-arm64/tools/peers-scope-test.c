 ...
 *
 * Ported byte-for-byte from asm-x86_64/tools/peers-scope-test.c (the fix is identical logic,
 * transliterated per-ISA — see this repo's AGENTS.md ISA-port lineage note); this file is
 * plain, target-agnostic C, so no aarch64-specific changes were needed beyond the build/run
 * path (cross `aarch64-linux-gnu-gcc` + `qemu-aarch64-static`, matching `make host`).
 *
