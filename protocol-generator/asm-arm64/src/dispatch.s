...
.Lcs_oversize:
	...
	mov  w20, w9                     // remaining bytes to drain (zero-extended)
	adr_l x9, g_rid_len
	str  xzr, [x9]
.Lcs_drain:
	cbz  x20, .Lcs_drained
	mov  x2, x20
	cmp  x2, #1048576                 // 0x100000 (imm12<<12), accepted by cmp
	b.ls .Lcs_drain_sz
	movz x2, #0x10, lsl #16          // 1048576
.Lcs_drain_sz:
	mov  x0, x19
	adr_l x1, b_req
	bl   read_full
	cmp  x0, #0
	b.le .Lcs_done                   // EOF mid-drain → peer gone
	sub  x20, x20, x0
	b    .Lcs_drain
.Lcs_drained:
	mov  x0, #413
...
derive_handler:
	...
	str  x21, [sp, #32]            // x21=uri len/remaining(r13)
...
.Ldh_scan:
	cbz  x21, .Ldh_ret              // no '/', keep default
	ldrb w9, [x19]
	cmp  w9, #0x2f
	b.eq .Ldh_found
	add  x19, x19, #1
