#!/usr/bin/env bash
# run-s3.sh — S3 (two-peer loopback interop) gate for the node-red peer.
#
...
echo "=============================================================="
echo " S3 peer-machinery gate — entity-core-protocol-node-red"
...
if ! grep -qF 'TS="/work/protocol-generator/typescript"' "$PEER/run-s4.sh"; then
  echo "    RED: node-red no longer builds its engine from ../typescript."
