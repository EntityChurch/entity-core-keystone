#!/usr/bin/env bash
# run-s2.sh — S2 (codec / crypto-agility) gate for the node-red peer.
#
# THIS PEER HAS NO CODEC OF ITS OWN, AND THAT IS A CLAIM THIS SCRIPT CHECKS RATHER THAN
# ASSERTS. It is a thin flow-runtime seam over `typescript`: its harness builds and loads the
...
echo "=============================================================="
echo " S2 codec gate — entity-core-protocol-node-red"
...
if ! grep -qF 'TS="/work/protocol-generator/typescript"' "$PEER/run-s4.sh"; then
  echo "    RED: node-red no longer builds its engine from the typescript peer."
