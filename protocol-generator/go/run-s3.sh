#!/usr/bin/env bash
# S3 peer machinery — the two-peer loopback smoke (src/peer/smoke_test.go).
#
...
podman run $PODMAN_RUN_CAPS --rm --network=none \
  ...
  bash -lc 'cd /work/protocol-generator/go/src && CGO_ENABLED=0 GOFLAGS=-mod=mod go test ./peer -run 'Smoke' -v -count=1'
