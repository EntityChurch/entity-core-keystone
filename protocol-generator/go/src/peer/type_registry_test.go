...
// TestCoreTypeRegistryByteIdentical renders every §9.5 floor type from the
// in-code model and asserts its content_hash (sha256 digest, the trailing 32
// bytes after the 0x00 format byte) is byte-identical to the canonical
// type-registry vectors. This is the S8 golden-file gate: a divergence means a
// field shape (key set, optional flag, carrier, layout) drifted from the floor.
func TestCoreTypeRegistryByteIdentical(t *testing.T) {
	defs := coreTypeDefs()
	if len(defs) != 53 {
		t.Fatalf("core type floor = %d, want 53", len(defs))
	}
	if len(floorTypeHashes) != 53 {
		t.Fatalf("floor hash table = %d, want 53", len(floorTypeHashes))
	}
	seen := map[string]bool{}
	for _, td := range defs {
		want, ok := floorTypeHashes[td.name]
		if !ok {
			t.Errorf("%s: not in the floor hash table", td.name)
			continue
		}
		seen[td.name] = true
		e := td.entity()
		// e.Hash = 0x00 format byte ‖ 32-byte SHA-256 digest.
		if len(e.Hash) != 33 || e.Hash[0] != 0x00 {
			t.Errorf("%s: unexpected content_hash framing %x", td.name, e.Hash)
			continue
		}
		got := hex.EncodeToString(e.Hash[1:])
		if got != want {
			t.Errorf("%s: content_hash mismatch\n  got  %s\n  want %s", td.name, got, want)
		}
	}
	for name := range floorTypeHashes {
		if !seen[name] {
			t.Errorf("%s: in hash table but not rendered by coreTypeDefs()", name)
		}
	}
}
