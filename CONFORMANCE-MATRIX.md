...
**Cohort:** **46 peers in the tree · 45 measured · all 45 at ONE pin — the 755-check set `95edd774…` · 41 of the 45 at 0-FAIL (2026-08-29).** Every number in §1 is a fresh measurement at that pin — nothing is carried forward, and the two-pin split this file carried earlier today is gone. **The pin is a content digest, not a commit** — see [The pin](#the-pin--content-anchored-and-why-there-is-no-commit-here) for the full anchor set and how to reproduce a number without access to our `dev` branch.

| | Peers |
|---|---|
| **0-FAIL @ the 755-check pin** (41) | `go` `haskell` `lean` `ocaml` `swift` (**tier M1**, 5/5) · `common-lisp` `csharp` `elixir` `java` `kotlin` `python` `rust` `typescript` (**tier M2**, 8/8) · `ada` `c` `cpp` `crystal` `dart` `julia` `nim` `odin` `php` `prolog` `ruby` `zig` (**tier M3**, 12/13) · `datalog` `forth` `fortran` `io` `oz` `pd` `rexx` `rust-wasm` `rust-wasm-wasmtime` `smalltalk` `sql` `tcl` `unison` `wasm-wat` (**probe**, 14/18) · `node-red` `turbowarp` (**exploratory**, 2/2) |
...
*This table was one pin behind between 2026-08-28 and 2026-08-29 — it still read `0-FAIL (13)` with 23 fixed peers filed under `3F`, four lines below a headline that already said 39. The per-peer rows in §1 were correct throughout; the summary above them was not. **A summary table is a second copy of a number and rots on its own schedule** — the same self-contradiction `README.md` carried in the 2026-08-23 sweep, in the same shape, two documents later.*

**The headline is one sentence: the CAP propagation is done — 41 of 45 measured peers pass `--profile core` at 0-FAIL, and the four that do not are four separate problems rather than one.** `--profile core` gained three `capability` checks at the 2026-08-21 re-pin and every unfixed peer failed exactly those three; that uniformity was the finding, and it held all the way through. The fix shape did not vary across **thirty-six** languages — the same five places every time (capability mint, codec salvage decode, wire `400`, read loop, policy lookup) — and that invariance is the evidence the spec reading is right, not merely that the tests pass.

**What remains is NOT the CAP feature.** `cobol` 30F (the trio plus its standing 27-FAIL liveness cascade) and the `asm-x86_64`/`asm-arm64`/`riscv64` trio (INVALID measurements — §1a). `apl` is unmeasurable and upstream-blocked. Each is its own investigation; none is the mint ceiling.
...
**The committed per-peer records now agree with §1 for every peer we publish.** All 40 publishable peers' `protocol-generator/<lang>/status/CONFORMANCE-REPORT.{md,json}` are at the pinned 755-check set and each reproduced its §1 row exactly — 13 re-measured on 2026-08-22, 26 as the propagation reached them on 2026-08-28, and `turbowarp` on 2026-08-29 (measured twice, by two destinations of the same tool, agreeing byte-for-byte); `make lint` runs `tools/check-set-gate.py --tracked`, which fails if a peer published as 0-FAIL carries a committed report from an older check set. **The remaining 6 peers' committed reports are behind the pin** (`cobol` and `wasm-wat` at the retired 740-check set; `apl` and the asm/ISA trio at 682) — they are behind on the *fix*, not merely on the paperwork, and the gate reports them without failing on them (§3). *(Until 2026-08-22 this was true of all 45, including the publishable ones: a clone showed each peer's own report contradicting its row here. §1 was never wrong; nothing gated those files.)* **§1 remains authoritative for the cohort; a peer's `status/` report is that peer's own last measurement.** Refresh one with `tools/run-cohort-census.sh --to-status <peer>` — it is a **measurement**, never a copy of the census JSON.
...
| **COBOL** | M3 | v0.8.0 | `95edd774…` | 755 · **30F** — 279P/335W/30F/111S | **FFI-hybrid** (COBOL value-codec + C-ABI) | **FFI** — `libentitycore_codec` (libsodium) | deferred (libsodium has no Ed448) | `make dist`, `0.1.0-pre` |
...
² **Every `--profile core` cell is a fresh measurement at the 755-check pin `95edd774…` (2026-08-21)** — `total · NF — P/W/F/S`, from one centralized `tools/run-cohort-census.sh` run over all 45 measurable peers. Nothing in this column is carried forward from an earlier pin: `755 ≠ 740`, so a figure from the retired 740-check pin (`8537d875…`) is not comparable to one here even when the F-count matches. The **three** peers that did not execute the full 755 are quarantined in §1a rather than listed with a score. *(Four, until `csharp` was fixed 2026-08-22 — §1c.)*
...
⁷ **`wasm-wat`'s 3F is not the CAP feature, and its number went UP because a measurement got HONEST.** Until 2026-08-29 this peer was the only one in the cohort whose `run-s4.sh` never provisioned `~/.entity/peers/conformance/keypair`, so four checks could not build their fixtures and SKIPped — reading as substrate limits rather than a missing setup file. Provisioning it (the peer hardcodes that same seed already) resolved three and converted the fourth into a real FAIL: it refuses a valid 2-of-3 multisig capability it co-signed, because `$verify_cap` hard-refuses a map-valued `granter`. Tracing the remaining `capability` refusals then showed they are not the §5.6 ceiling at all — the peer has **no §5.5 delegation chain**, so a delegated cap is refused two gates before the mint is reached. **304P/339W/2F/110S → 306P/339W/3F/107S: four fewer skips, two more passes, one more disclosed failure.** §3 carries both rows. Do not read the 2F→3F move as a regression — nothing about the peer changed.
...
**Status at the 755-check pin: `asm-x86_64` · `asm-arm64` · `riscv64` (714/755).
...
| **`wasm-wat` — §5.5 delegation chains are ABSENT, and the 2F was hiding it** | 1 peer | Medium — larger than this row said until 2026-08-29 | **The estimate below was wrong and is corrected in place because it was an estimate, not a measurement.** It read: *"the mint is a literal `(call $w_map (i64.const 4))` with fixed memory offsets, so adding `expires_at` is an arity + data-segment edit rather than a code edit … neither is hard."* Measured 2026-08-29 by tracing the actual refusal: **`wasm-wat` implements no capability chain at all.** `$verify_cap` requires `granter == this peer` (root-trust, depth-1 only) and `$serve_delegate` answers `501` to any request carrying a `parent`. All three outstanding `capability` results present a **delegated** cap — instrumented at the refusing gate, the presented `granter` is the caller's identity hash rather than this peer's, and `parent` is present on **4 of 4** refused tokens. So CAP-5 and CAP-6 never reach the mint at all; they are refused two gates earlier, and the §5.6 ceiling they are named for is not what fails. This is the `sql`/`datalog` shape at a larger scale — **a blanket wrong denial standing in for an entire unimplemented feature** — and it stayed invisible because the peer scored a respectable 2F. Real scope: §5.5 chain collection + per-link signature/grantee/temporal/attenuation validation, §3.6 K-of-N quorum roots, the §5.6 ceiling, and CAP-6a representability — in hand-authored WAT. Its own session. |
...
| **`turbowarp` CAP-2/3** | 1 peer | Low — never gates | The block-interpreter probe (§1 ‡), not a deployable peer. The CAP trio in Scratch blocks. |
...
### Current state — 2026-08-22 @ the 755-check pin `95edd774…` (post-M1-fix, post-full-census, post-M2-fix)
...
| **M3** | **12 / 13** | **Fixed 2026-08-28.** Only `cobol` remains, and its 30F is the CAP trio plus a standing 27-FAIL liveness cascade — a separate investigation |
| **probe** | **13 / 18** | **Fixed 2026-08-28** (11 peers). Remaining: `asm-x86_64` `asm-arm64` `riscv64` INVALID (§1a) · `wasm-wat` 2F · `apl` unmeasurable |
| **exploratory** | **1 / 2** | `node-red` 0F (it was never broken — stale `dist/`, §3). `turbowarp` 3F. Never gates (§1 ‡) |

**Every tier is current on the pin — nothing is stale, and as of 2026-08-28 the CAP debt is
closed.** What the lower tiers carried was one debt (§5.6's MIN_DEFINED mint ceiling, absent
everywhere), and propagating it took the cohort from 13 publishable to **41 of 45**. What remains is
six *different* problems, not one: `cobol`'s liveness cascade, the asm/ISA connection-pressure
family, two unstarted hand-authored substrates, and one unmeasurable peer. *(Before this pass this
paragraph read "25 of the 29 scored unfixed peers fail nothing but the new CAP checks"; the finding
was right and is now history.)*
