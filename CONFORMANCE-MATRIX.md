...
|------|:----:|:----:|---------------|:----------------:|-------|----------------------------------|-------------------------|---------|
| **OCaml** | **M1** | `0.8.2.31` ¹⁴ | `7aa6f3de…` | 778 · **0F** — 335P/336W/0F/107S ⁵ | native hand-rolled | native — mirage-crypto-ec + digestif | **FFI-hybrid** (opt-in `entitycore_agility`) | opam, `0.1.0-pre` |
| **Swift** | **M1** | `0.8.2.31` ¹⁴ | `7aa6f3de…` | 778 · **0F** — 335P/336W/0F/107S ⁵ | native hand-rolled | native — swift-crypto | deferred (→ FFI when scoped) | SPM, `0.1.0-pre` |
| **Haskell** | **M1** | `0.8.2.31` ¹⁴ | `7aa6f3de…` | 778 · **0F** — 335P/336W/0F/107S ⁵ | native hand-rolled | native — crypton | **native** — crypton (Ed448) | Cabal, `0.1.0-pre` |
| **Go** (clean-room) | **M1** | `0.8.2.31` ¹⁴ | `7aa6f3de…` | 778 · **0F** — 335P/336W/0F/107S ⁵ | native hand-rolled | native — stdlib `crypto/ed25519` | deferred (→ FFI when scoped) | Go module, `0.1.0-pre` |
| **Lean** | **M1** | `0.8.2.31` ¹⁴ | `7aa6f3de…` | 778 · **0F** — 335P/336W/0F/107S ⁵ | **pure-Lean proven core** + FFI crypto | **FFI** — C-ABI `ec_ed25519_*` | FFI (deferred) | Lake, `0.1.0-pre` |
| **C#** | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 336P/335W/0F/107S ✅ *(was an INVALID MEASUREMENT at 698/755 with 9 starved categories; fixed 2026-08-22 — §1c. Run time 18m20s → **7.2s**.)* | native (Cbor Ctap2 + handroll) | native — NSec | managed — BouncyCastle | NuGet, `0.1.0-pre` |
| **TypeScript** | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 84F = 3 real + 81 cascade; fixed 2026-08-22 — §1b. The one WARN off the 336P row is `concurrency/t1_1_concurrent_demux`, the timing-ratio check — see footnote ¹¹.)* | native (cborg + handroll) | native — @noble | managed — @noble | npm, `0.1.0-pre` |
| **Java** | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-22)* | native hand-rolled | native — JDK SunEC | JDK / BouncyCastle | Maven, `0.1.0-pre` |
| **Kotlin** | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-22)* | native hand-rolled | native — JDK SunEC | deferred (→ JDK SunEC / BouncyCastle) | Gradle→Maven Central, `0.1.0-pre` |
| **Elixir** | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-22)* | native hand-rolled | native — OTP `:crypto` | **native** — OTP `:crypto` | Hex, `0.1.0-pre` |
| **Common Lisp** | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ *(was 3F; CAP fix 2026-08-22. One WARN off the cohort: `concurrency/t1_1_concurrent_demux` reports no parallel speedup under load — the check names itself informational, **not** a §6.11 violation.)* | native hand-rolled | native — ironclad (pure-Lisp) | **native** — ironclad (pure-Lisp) | ASDF/Quicklisp, `0.1.0` |
| **Rust** (clean-room) | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-22)* | native hand-rolled | native — ed25519-dalek + sha2 | deferred (→ FFI when scoped) | crates.io, `0.1.0-pre` |
| **Python** (clean-room) | M2 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-22)* | native hand-rolled | native — `cryptography` (OpenSSL) | **native** — `cryptography` (Ed448) | PyPI, `0.1.0` |
| **Zig** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 336P/335W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28. 314P/336W until 2026-09-02: `r3_connection_flood` WARN→PASS on a §4.10(c) admission bound — see §3, and note the peer carries a measured intermittent abort)* | native (std-only) | native — `std.crypto` | deferred | source, `0.1.0-pre` |
| **C** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | native hand-rolled | native — libsodium | deferred (libsodium has no Ed448) | `make dist` + pkg-config |
| **C++** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | native hand-rolled | native — libsodium | deferred (libsodium has no Ed448) | CMake pkg + vcpkg + conan, `0.1.0-pre` |
| **Ada** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 336P/335W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | native hand-rolled | native — libsodium (C binding) | deferred (libsodium has no Ed448) | Alire (optional), `0.1.0-pre` |
| **Ruby** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | native hand-rolled | native — stdlib `openssl` | **native** — stdlib `openssl` | RubyGems, `0.1.0.pre` |
| **Crystal** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | native hand-rolled | native — libsodium (direct `lib`/`fun` C binding) | deferred (libsodium has no Ed448; → FFI) | source, `0.1.0-pre` |
| **Odin** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | native hand-rolled | **native — pure-Odin `core:crypto`** (Ed25519 + SHA-2, FFI-free) | deferred (`core:crypto` has no Ed448; → FFI-hybrid) | source (`make dist`), `0.1.0-pre` |
| **Prolog** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | **FFI** (C-ABI) | **FFI** — C-ABI (library(crypto) has no Ed25519) | FFI | SWI pack, `0.1.0` |
| **PHP** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | native hand-rolled | native — ext-sodium (libsodium) | deferred (ext-sodium has no Ed448; → FFI) | Composer, `0.1.0-pre` |
| **Dart** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28. The extra WARN against the modal row is `concurrency/t1_1_concurrent_demux`, the timing-ratio check — see footnote ¹¹.)* | native hand-rolled | native — cryptography_plus (pure-Dart) | deferred (→ FFI when scoped) | pub.dev, `0.1.0-pre` |
| **COBOL** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 30F: 24 were a cascade behind one unchecked `MOVE` of wire data into a fixed field, which glibc's fortify check turned into a process kill. §3. Carried one extra skip until 2026-09-04 — `t1_3`, a 264 KB frame against a 64 KiB cap — and now sits on the cohort's modal row; the capacity became affordable only after `cbor-canon` stopped buffering map values, 2.13 MB → 65.8 KB per call per level.)* ⁸ | **FFI-hybrid** (COBOL value-codec + C-ABI) | **FFI** — `libentitycore_codec` (libsodium) | deferred (libsodium has no Ed448) | `make dist`, `0.1.0-pre` |
| **Tcl** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | **FFI-hybrid** (pure-Tcl canonical CBOR + C-ABI) | **FFI** — `libentitycore_codec` (C-shim, libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | git + `pkgIndex.tcl`, `0.1.0-pre` |
| **Rexx** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | **FFI-hybrid** (pure-Rexx **decimal-model** canonical CBOR + C-ABI) | **FFI** — `libentitycore_codec` via the `ecnet` co-process daemon (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | `make dist` (git + tarball), `0.1.0-pre` |
| **Fortran** | probe | `0.8.2.25` ¹³ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28. 2026-09-16: swept to `0.8.2.25` — CAP-6a WARN→PASS, its 3 transport-drop variants now answer §6.3's disposition, and the suite went 62.3 s → 2.1 s because each dropped frame had been billing the caller a full timeout.)* | **FFI-hybrid** (pure-Fortran **signed-carrier uint64** canonical CBOR value codec + C-ABI) | **FFI** — `libentitycore_codec` bound direct via `iso_c_binding`, no C wrapper (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | `make dist` (git + tarball), `0.1.0-pre` |
| **Forth** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ *(was 4F; CAP trio + CAP-2 withdrawal form)* | **FFI-hybrid** (pure-Forth **native-float-bits** canonical CBOR + C-ABI) | **FFI** — `libentitycore_codec` via in-process `libcc` `c-function` (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | `make dist` (git + tarball), `0.1.0-pre` |
| **Smalltalk** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ *(was 4F; CAP trio + CAP-2 withdrawal form. 2026-09-14: CAP-6a WARN→PASS — its 3 transport-drop variants now answer §6.3's status, F79.)* | **FFI-hybrid** (pure-Smalltalk canonical CBOR + C-ABI) | **FFI** — `libentitycore_codec` via in-process UFFI `ffiCall:module:` (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | git + Metacello/Tonel, `0.1.0-pre` |
| **APL** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ ¹⁰ *(carried as "not measured — upstream-blocked" until 2026-08-30, on a toolchain claim that was false; 8 real FAILs then closed in one session — §1d)* | **FFI-hybrid** (pure-APL **array value-model** canonical CBOR codec + C-ABI) | **FFI** — `libentitycore_codec` via a GNU APL `⎕FX` native fn (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | `make dist` (git source), `0.1.0-pre` |
| **asm-x86_64** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ ⁹ *(the disclosed `t1_3_no_head_of_line` skip is CLOSED — a 64 KiB ECF scratch in the peer's own native codec, not the payload; see §1. Was an INVALID MEASUREMENT — §1a; that cause was a §4.9(c) silent drop, not connection pressure.)* | **native (L2)** hand-written x86-64 asm — envelope/data-map CBOR **+ canonical ECF codec** (shortest-float ladder, key-sort, `ec_content_hash`, peer-id format/parse); only crypto is FFI | **FFI** — Ed25519 + SHA-256 via `libentitycore_codec` (libsodium); the canonical codec is native asm (L2 boundary) | deferred (→ FFI, C-ABI `ec_ed448_*`) | source (`make host`), `0.1.0-pre` |
| **asm-arm64** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ ⁹ *(was an INVALID MEASUREMENT — §1a; the cause was a §4.9(c) silent drop, not connection pressure.)* | **native (L1)** aarch64 GAS transliteration of the x86-64 peer — envelope/data-map CBOR + dispatch interior in hand-written asm; canonical codec + crypto via `libentitycore_codec` (cross-built for aarch64); run under `qemu-aarch64-static` | **FFI** — Ed25519 + SHA-256 via `libentitycore_codec` (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | source (`make host`), `0.1.0-pre`|
| **riscv64** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ ⁹ *(was an INVALID MEASUREMENT — §1a; the cause was a §4.9(c) silent drop, not connection pressure.)* | **native (L1)** RV64GC GAS port off the arm64 template via the shared generic syscall table; canonical codec + crypto via `libentitycore_codec` (cross-built for riscv64); run under `qemu-riscv64-static` | **FFI** — Ed25519 + SHA-256 via `libentitycore_codec` (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | source (`make host`), `0.1.0-pre`|
| **wasm-wat** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ ⁷ | **seam-hybrid** (hand-authored WAT peer + wire codec; Rust codec compiled to `wasm32-wasip1`, wasm-merged as the seam) | **seam** — `entitycore_codec.wasm` (Rust→wasm, Ed25519 + SHA-256) | deferred (→ codec seam `ec_ed448_*`) | source (`make peer`), `0.1.0-pre` |
| **rust-wasm** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(census read 3F against a `peer.wasm` seven days older than the `../rust` source it compiles; already fixed by inheritance — §3. 2026-09-14: CAP-6a WARN→PASS — the §6.3 answer does NOT come by inheritance, because this seam reimplements the read loop, F79.)* | **native (inherited)** — the `../rust` peer's hand-rolled ECF codec cross-compiled UNMODIFIED to `wasm32-wasip1`; only a 336-line `poll_oneoff` transport seam is wasm-specific | **native** — ed25519-dalek + sha2 (compile to wasm cleanly, no seam) | deferred (→ codec seam `ec_ed448_*`) | source (`make peer`), `0.1.0-pre` |
| **rust-wasm-wasmtime** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(as `rust-wasm` — stale artifact, already fixed by inheritance — §3. 2026-09-14: CAP-6a WARN→PASS, F79.)* | **native (inherited)** — the SAME `rust-wasm` `wasm32-wasip1` module, run under **wasmtime AOT** (`wasmtime compile` → `.cwasm`); only the socket seam differs (host-preopened `-S tcplisten` + standard wasip1 `sock_accept`/`poll_oneoff`, the `wasi` crate) | **native** — ed25519-dalek + sha2 (compile to wasm cleanly, no seam) | deferred (→ codec seam `ec_ed448_*`) | source (`make aot`), `0.1.0-pre` |
| **Julia** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | **native** hand-rolled (**multiple-dispatch** canonical CBOR) | **native** — system libsodium via `ccall` + `SHA` stdlib (Ed25519 + SHA-256; native-audited-lib tier, NOT the C-ABI) | deferred (→ opt-in FFI, C-ABI `ec_ed448_*`; libsodium has no Ed448) | Pkg (Project.toml + git), `0.1.0-pre` |
| **Nim** | M3 | `0.8.2.31` ¹⁴ | `7aa6f3de…` | **778 · 0F** — 336P/335W/0F/107S ✅ *(was 4F. Its §5.6 ceiling already existed and was wrong three ways — §3. `handshake_nonce_single_use` WARN→PASS 2026-09-07: routing the connect handler in ANY connection state, per §4.7 row 10, made a dead RT-6 arm live — a replayed authenticate now answers the pinned 401 invalid_nonce instead of reaching the generic 401 missing_author.)* | **native** hand-rolled (**compile-time macro/template** canonical CBOR) | **native** — libsodium via `{.importc.}` C interop (Ed25519 + SHA-256) | deferred (libsodium has no Ed448) | nimble (git-indexed), `0.1.0-pre` |
| **Oz / Mozart** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 328P/343W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | **FFI-hybrid** (pure-Oz canonical CBOR value codec + C-ABI; bignum ints, IEEE floats as exact bit-patterns) | **FFI** — `libentitycore_codec` via the `entity-codec-daemon` `Open.pipe` co-process (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | `make dist`, `0.1.0-pre` |
| **Io** | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 334P/337W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28. Went **28F on 2026-09-03** when the `-reference-peer` fold first ran the §6.11 origination check, and back to 0F the same day once the cause was found — see §1e. The FAIL was real and the fix is structural, not a re-run: **pre-fix 3 of 6** full-suite runs failed, **post-fix 0 of 12**.)* | **FFI-hybrid** (pure-Io canonical CBOR + C-ABI; `EcBig` uint64 carrier for the double number model) | **FFI** — `libentitycore_codec` via the in-process `EntityCodec` C addon (libsodium) | deferred (→ FFI, C-ABI `ec_ed448_*`) | `make dist`, `0.1.0-pre` |
| **SQL** (authority-as-query) | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 2F. NOT a plain CAP fix: the §5.5a handlers over-scoping that caused it was also masking a missing chain-attenuation rung and a missing §6.2 mint-bound — §3.)* | **seam-hybrid** — §5.2 ladder / §5.5 chain-walk (recursive CTE) / §3.6 K-of-N (`HAVING count DISTINCT`) / §6.6 (`ORDER BY length DESC`) **authored as real SQL** (`src/sql/`); thin C host owns sockets/CBOR/crypto/store over the C-ABI | **FFI** — `libentitycore_codec` (libsodium); crypto callable *from* SQL via `sqlite3_create_function` | deferred (→ FFI, C-ABI `ec_ed448_*`) | source (`make dist`), `0.1.0-pre` |
| **Datalog** (authority-as-query) | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 336P/335W/0F/107S ✅ *(was 3F. Like `sql`, the fix exposed a missing §5.5a frame isolation the earlier wrong denial had been answering — §3. 2026-09-14: CAP-6a WARN→PASS — its read loop dropped a refused frame instead of answering it, F79.)* | **seam-hybrid** — §5.5 delegation as recursive **Ascent rules** to least fixpoint (SecPAL/Binder shape), §5.2 verdict as a derived `allow` fact, K-of-N counting aggregate, §6.6 stratified negation; pure-Rust host asserts `verified_signer` facts over the C-ABI | **FFI** — `libentitycore_codec` (libsodium); Ascent never touches a byte | deferred (→ FFI, C-ABI `ec_ed448_*`) | source (`cargo build --release`), `0.1.0-pre` |
| **Node-RED**‡ | exploratory | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(census read 3F against a stale `dist/`; already fixed by inheritance from `typescript` — §3. 2026-09-14: CAP-6a WARN→PASS — the flow collapsed a strict-decode refusal into its drop node, F79.)* | **interop** (delegates the TS peer's canonical CBOR + §6.5 engine) | **interop** — TS peer `@noble` (Ed25519 + SHA-256) | deferred (→ TS `@noble` seam) | not a peer (illustrative) |
| **TurboWarp**‡ | exploratory | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-29 — the ordinary fix shape, the 37th language it has not varied in. `capability/ingest_rejects_unrepresentable_expiry` WARN → **PASS** on 2026-09-16: the 0.8.2.25 §4.11 work made this peer answer a CODED frame where it had dropped the connection, and CAP-6a scores a transport-drop refusal as WARN precisely because a drop is a refusal but not the §5.2 disposition. **This is the only real check the whole 46-peer 0.8.2.25 sweep moved at this pin** — 3 of 35 788 severities moved across all 46 reports and the other two were the footnote-¹⁰ timing ratio.)* | **ALL FIVE handler bodies (§4 connect / echo / §6.3 tree / §6.2 handlers / §6.2 capability) + §6.5 dispatch + §5.2 verify AUTHORED in Scratch** — a dispatch spine routing to one `define dispatch-<handler>` custom block each; only socket/CBOR/crypto/store via the `ecutils` seam | **seam** — bundled `@noble` (Ed25519 + SHA-256) | deferred (→ bundled `@noble`) | not a peer (illustrative; real-VM confirmation pending) |
| **Pure Data**‡ | probe | `0.8.2.25` | `7aa6f3de…` | **778 · 0F** — 332P/339W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28)* | **§6.5 dispatch spine + §5.2 verify ladder + §4.6 auth ladder + §6.6 walk AUTHORED on the canvas** (pattern-first spine, one named unit per handler); bytes/CBOR/crypto/store/**TCP transport** in the `[ecodec]` C external (stock `[netreceive]` broadcasts replies — A-PD-002) | **FFI** — `libentitycore_codec` (libsodium) via `[ecodec]` | deferred (→ FFI, C-ABI `ec_ed448_*`) | not packaged (visual-paradigm probe on the real Pd runtime) |
| **Unison** | probe | `0.8.2.25` ¹³ | `7aa6f3de…` | **778 · 0F** — 335P/336W/0F/107S ✅ *(was 3F; CAP fix 2026-08-28. 2026-09-16: swept to `0.8.2.25` — 0 checks moved, which is the point: the rules are ungated by this check set.)* | **native** hand-rolled canonical ECF on **UCM runtime builtins — ZERO third-party dependencies** (CBOR/base58/LEB128 hand-rolled; `Float.toRepresentation` gives raw IEEE bits for the shortest-float ladder) | **native** — builtin `crypto.Ed25519.sign/verify.impl` + `crypto.hashBytes Sha2_256`; **key derivation HAND-WRITTEN in pure Unison** (GF(2²⁵⁵−19) base-2¹⁶ limb arithmetic + twisted-Edwards scalar mult — UCM ships sign/verify but NO keygen, A-UN-009) | deferred — **no C-FFI hatch** (managed runtime; Ed448/SHA-384 are not builtins, so the `libentitycore_codec` path other peers used is *structurally* unavailable) | Unison Share deferred, `0.1.0-pre` |
...
¹² **`Spec` is the spec revision the peer has been SWEPT to — and until 2026-09-16 this column had no definition anywhere and published `v0.8.0` on all 46 rows.** It was recording the snapshot each peer was originally GENERATED against, which stopped being the interesting fact the first time a revision was swept across the cohort; with no footnote saying so, a reader saw `v0.8.0` and correctly concluded the peer implements v0.8.0. **That was wrong for all 46 rows** — the `0.8.2.25` sweep closed at 46 of 46 on 2026-09-15 (`docs/STATUS.md`). Corrected after `entity-system-conformance` measured it and said so, which is the useful direction for a counterpart to be wrong in: they own the census that reads this column. **This column was HAND-MAINTAINED and no gate watched it**, which is exactly how it drifted — and the sentence that stood here said the durable fix was *"owed rather than done"*, ending **"a value that is correct today and ungated is a value that is correct today."** It was wrong on two rows within the day (footnote ¹³). ✅ **DONE 2026-09-16: `tools/peer-tiers.tsv` carries a `spec_pin` column and `tools/spec-pin-gate.py` runs in `make lint`.** The roster is the machine-readable source, this column mirrors it, and **the gate fails on any disagreement between the two** — so the drift that produced this footnote cannot recur silently. `unknown` is a spellable value and a blank cell is an ERROR (they are different claims). Peers behind the cohort revision are *reported with a count*, not failed, so tracked backlog cannot hold the gate red. The sweep reconciliation — `spec-pin-gate.py --since <ref>`, the **set difference** that catches a peer no tranche touched — is run when a sweep closes rather than in `make lint`, because it needs a range only the sweep can name; re-run against this sweep's start it names `fortran` and `unison` and nothing else.

¹⁴ ⚠ **A `0.8.2.31` pin means the peer was SWEPT to `.31`, NOT that every `.31` rule is implemented — six behaviour items are outstanding COHORT-WIDE and are enumerated here rather than left to be discovered.** The 26 maintained-tier rows (M1/M2/M3) carry the `0.8.2.25 → 0.8.2.31` sweep landed 2026-09-16/17: §7a.1's PLURAL reentry carriers with the singular spelling accepted as an array of one, §7a.2a's merged verification bundle, §6.8's narrow scaffold grant, and §1.4's **PD-2 gate** (`check_permission` before a locally-originated sub-dispatch leaves the peer, all four dimensions on the executing handler's own grant, with a target-minted credential relaxing Dimension 4 and only Dimension 4). The 20 probe/exploratory rows are honestly at `0.8.2.25` and the gate reports the gap with a count rather than failing — a peer behind the cohort is tracked backlog, and `spec_pin` is what makes it countable. **Pinning 26 freshly-swept peers at `.25` would have hidden the arc; pinning them at `.31` with no list would have hidden the gaps. What is owed, measured rather than remembered:** **(a)** `resolve_peer_scope` (`.29`) — §1.5 form canonicalization of the `peers` dimension before the literal match, with the fail-closed asymmetry (unresolvable `include` → drop it, unresolvable `exclude` → refuse the grant); **no peer names the function** (`grep` over all 46 source trees returns 0) and nothing drives the property, so this reads *not-driven* rather than *absent*. **(b)** §7a.1b's per-call **`deadline_ms` `[MUST]`** — a wire FIELD, so the name IS the property: **0 of 46**, and it reached us through no packet at all (`F88`). **(c)** §4.6 step 3 tests the BINDING, not the FORM — a non-canonical but well-formed `peer_id` that decodes to the presented `public_key` MUST NOT be refused `401 identity_mismatch`; not driven. **(d)** §4.11's bounded close — MUST NOT close while any admitted request is in flight; not driven. **(e)** tag policy at ANY NESTING DEPTH with tag 55799 a member — this is `pa-probe`'s `D5` arm, already on the owed list and now normative. **(f)** §6.2/§6.5/§6.7: `501` is reachable only AFTER `check_permission`, so unauthorized-and-unimplemented answers `403` — a CONFIDENTIALITY property (the reverse order lets a caller enumerate a handler's operation set). The candidate oracle gates it as `undefined_op_unauthorized_is_403_not_501`; **the `go` vanguard PASSes it and the cohort is unknown until driven.** **None of the six is gated by the pinned 778-check set**, which is why each needs naming here: a `0F` row is silent about every one of them, and that silence is the thing this footnote exists to break.

¹³ ✅ **CLOSED 2026-09-16 — both peers are swept and this column is true again; the history below is kept because the FAILURE MODE is the finding, not the two rows.** Each went 6 `arc-probe` rows owed → **0 of 15** and 5 of 6 §4.11 arms owed → **0 of 6**, census measured twice per peer so each rule half is attributable: `unison` **0 of 778** severities on both, `fortran` **0** then exactly **one** (CAP-6a WARN→PASS, the §6.3 salvage turning three transport-drops into the disposition; its suite also went 62.3 s → 2.1 s, because a dropped frame bills the caller a full read deadline). **And the reconciliation that proves it failed first, on us:** `spec-pin-gate --since` selects sweep commits by SUBJECT and its pattern was `^(sweep tranche|vanguard)` — the tail of a sweep is not a numbered tranche, so it accused the two commits that closed the hole it exists to catch. Broadened with the hit list diffed (17 → 18 over the range, one addition, and it is the `fortran` sweep). ⬜ **What follows is the 2026-09-16 correction as written, preserved:** **`fortran` and `unison` were NEVER SWEPT to `0.8.2.25`, this column published them at it for one day, and footnote ¹²'s own closing sentence is what came true.** ¹² was written on 2026-09-16 to correct a `Spec` column that had published `v0.8.0` on all 46 rows, and it ended *"a value that is correct today and ungated is a value that is correct today."* It was wrong on two rows **the same day it was written**. Measured 2026-09-16 on a **single-age** roster run of `tools/arc-probe` (46 of 46 reported, report-age span **0.04 h**) plus `tools/pa-probe` on both peers: they owe **six `arc-probe` rows each** — `A1`/`A2`/`A3`/`A4` (the §3.3 effective-targets ladder) and `G2`/`G4` (§6.3's `check_path_permission` and the listing filter, i.e. **`F84`**) — where the other 44 conform, and **5 of 6 §4.11 arms each**, which is exactly the pre-sweep baseline `docs/STATUS.md` records for the nine peers that were swept last. Their last substantive commit is the `0.8.2.21` sentinel transcription (`0c4a537b`); **no `sweep tranche` or vanguard commit touches either of them** — verified by diffing the roster against every peer directory those commits modified: **44 swept, 46 on the roster.** **The failure mode is the durable half and it is not about these two peers: a sweep run tranche-by-tranche measures each peer as its tranche lands, so a peer that NO tranche touched is measured by nothing and appears in no tranche's report.** `output/scratch/arc/` then read as a cohort picture while spanning **29.5 hours**, with seven peers' reports older than their own sweep commit — and these two peers' reports were from the pre-sweep census two days earlier. **The control is one command and it is a set difference, not a measurement: diff the roster against the peers the sweep commits actually touched.** Corrected here rather than by sweeping them, because the sweep is real work and is named as owed — a false `0.8.2.25` on a published row is the thing that had to stop today. ✅ **The control is now executable and retrospectively proven: `tools/spec-pin-gate.py --since b71b940f~1` names these two peers and no others.** ⚠ **The scoping is the whole check and the obvious scoping is wrong** — a plain `<ref>..HEAD` diff reports **46 of 46** as touched, because the range also holds the closing re-measurement (which rewrites every peer's committed report) and the `0.8.2.21` transcription. The commits that constitute a sweep are selected **by subject**, which is a convention-keyed proxy and is documented as one: it answers *"did a sweep commit touch this peer"*, never *"was this peer measured"*. A peer that a sweep measured and found already conformant produces no diff — a real outcome, which is why that arm is an error **only until acknowledged** (`--ack-unchanged`), so the claim *"we measured it and it needed nothing"* has to be said out loud instead of being the silent default.
