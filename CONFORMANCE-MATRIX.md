...
| **Nim** | M3 | v0.8.0 | `c34abcae…` | **758 · 0F** — 318P/335W/0F/105S ✅ *(was 4F. Its §5.6 ceiling already existed and was wrong three ways — §3. `handshake_nonce_single_use` WARN→PASS 2026-09-07: routing the connect handler in ANY connection state, per §4.7 row 10, made a dead RT-6 arm live — a replayed authenticate now answers the pinned 401 invalid_nonce instead of reaching the generic 401 missing_author.)* | **native** hand-rolled (**compile-time macro/template** canonical CBOR) | **native** — libsodium via `{.importc.}` C interop (Ed25519 + SHA-256) | deferred (libsodium has no Ed448) | nimble (git-indexed), `0.1.0-pre` |
