...
**Cohort:** **46 peers in the tree · 45 measured · all 45 at ONE pin — the 755-check set `95edd774…` · 39 of the 45 at 0-FAIL (2026-08-28).** Every number in §1 is a fresh measurement at that pin — nothing is carried forward, and the two-pin split this file carried earlier today is gone. **The pin is a content digest, not a commit** — see [The pin](#the-pin--content-anchored-and-why-there-is-no-commit-here) for the full anchor set and how to reproduce a number without access to our `dev` branch.

| | Peers |
|---|---|
| **0-FAIL @ the 755-check pin** (39) | `go` `haskell` `lean` `ocaml` `swift` (**tier M1**, 5/5) · `common-lisp` `csharp` `elixir` `java` `kotlin` `python` `rust` `typescript` (**tier M2**, 8/8) · `ada` `c` `cpp` `crystal` `dart` `julia` `nim` `odin` `php` `prolog` `ruby` `zig` (**tier M3**, 12/13) · `datalog` `forth` `fortran` `io` `oz` `pd` `rexx` `rust-wasm` `rust-wasm-wasmtime` `smalltalk` `sql` `tcl` `unison` (**probe**, 13/18) · `node-red` (**exploratory**, 1/2) |
| **Larger** (1) | `cobol` — 30F (CAP trio + its standing 27-FAIL liveness cascade) |
| **3F** (2) | `wasm-wat` — hand-authored WebAssembly text. Was 2F until 2026-08-29, when provisioning the conformance keypair every other peer already had exposed a fourth defect; §5.5 delegation chains are absent entirely and that is what the CAP results are actually reporting (§3) · `turbowarp` — the CAP trio. Block-interpreter probe, **not a deployable peer**; never gates |
...
**The headline is one sentence: the CAP propagation is done — 39 of 45 measured peers pass `--profile core` at 0-FAIL, and the six that do not are six separate problems rather than one.** `--profile core` gained three `capability` checks at the 2026-08-21 re-pin and every unfixed peer failed exactly those three; that uniformity was the finding, and it held all the way through. The fix shape did not vary across **thirty-six** languages — the same five places every time (capability mint, codec salvage decode, wire `400`, read loop, policy lookup) — and that invariance is the evidence the spec reading is right, not merely that the tests pass.

**What remains is NOT the CAP feature.** `cobol` 30F (the trio plus its standing 27-FAIL liveness cascade), the `asm-x86_64`/`asm-arm64`/`riscv64` trio (INVALID measurements, a connection-pressure family — §1a), `wasm-wat` 3F and `turbowarp` 3F (both hand-authored/exploratory substrates; `wasm-wat`'s is **not** the CAP feature — see §3). `apl` is unmeasurable and upstream-blocked. Each is its own investigation; none is the mint ceiling.
...
| **wasm-wat** | probe | v0.8.0 | `95edd774…` | 755 · **3F** — 306P/339W/3F/107S ⁷ | **seam-hybrid** (hand-authored WAT peer + wire codec; Rust codec compiled to `wasm32-wasip1`, wasm-merged as the seam) | **seam** — `entitycore_codec.wasm` (Rust→wasm, Ed25519 + SHA-256) | deferred (→ codec seam `ec_ed448_*`) | source (`make peer`), `0.1.0-pre` |
...
everywhere), and propagating it took the cohort from 13 publishable to **39 of 45**. What remains is
