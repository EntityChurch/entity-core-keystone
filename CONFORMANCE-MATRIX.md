...
| **COBOL** | M3 | v0.8.0 | `c34abcae…` | **758 · 0F** — 317P/336W/0F/105S ✅ *(was 30F: 24 were a cascade behind one unchecked `MOVE` of wire data into a fixed field, which glibc's fortify check turned into a process kill. §3. Carried one extra skip until 2026-09-04 — `t1_3`, a 264 KB frame against a 64 KiB cap — and now sits on the cohort's modal row; the capacity became affordable only after `cbor-canon` stopped buffering map values, 2.13 MB → 65.8 KB per call per level.)* ⁸ | **FFI-hybrid** (COBOL value-codec + C-ABI) | **FFI** — `libentitycore_codec` (libsodium) | deferred (libsodium has no Ed448) | `make dist`, `0.1.0-pre` |
