...
**One row carries a disclosed gap behind a 0-FAIL verdict, named here rather than left to be found.** `cobol` executes **107** of the 756 checks as skips against the cohort's 106. The one extra is `concurrency/t1_3_no_head_of_line`, which stages a **264 109-byte** frame (measured on the wire, not taken from prose) against this peer's 65535-byte frame cap; it is refused with a correlated `413 payload_too_large`. That bound is spec-legal — §4.10(a) requires only that the maximum be **finite**, and the core protocol "places no restriction on entity size" — so the gap is that a conformant peer with a modest bound cannot be fully *measured*. Routed to architecture as `protocol-generator/shared/findings/conformance-payload-capacity-floor.md`.
...
| **COBOL** | M3 | v0.8.0 | `d30c3dd0…` | **756 · 0F** — 312P/337W/0F/107S ✅ *(was 30F: 24 were a cascade behind one unchecked `MOVE` of wire data into a fixed field, which glibc's fortify check turned into a process kill. §3.)* ⁸ | **FFI-hybrid** (COBOL value-codec + C-ABI) | **FFI** — `libentitycore_codec` (libsodium) | deferred (libsodium has no Ed448) | `make dist`, `0.1.0-pre` |
