...
| **`authz_peers_target_from_uri`** — WARN on **39** peers, and the "inconclusive by design" label is **WITHDRAWN** | 39 of 46 | **Medium** — it is a core §5.2 authority dimension, and the exculpation was wrong | **This row read *"WARN on every peer · Low — inconclusive by design · a single standalone peer cannot resolve `target_peer` against a synthetic foreign URI; needs a real two-peer harness. Not attempted since it was found 2026-08-16."* Measured across all 46 committed reports on 2026-08-30: it is not every peer and it is not inconclusive.** **6 peers PASS** — `asm-x86_64` `asm-arm64` `riscv64` `forth` `pd` `smalltalk` — with a real three-row verdict: *"§5.2 peers dimension correct: foreign-scoped grant ALLOWs the foreign target (P1 s=200), while local-scoped (P2 s=403 `capability_denied`) and absent-scoped rows deny."* So a standalone peer **can** decide it, and the harness is not the obstacle. The 39 WARN for a different reason: `go` — M1, the clean-room reference — returns **`404 handler_not_found` on all three rows**, i.e. it never reaches the authz ladder at all. That is unrouted, not undecidable. **This is the "an inbound claim that reduces your work is still an inbound claim" discipline failing on a claim we made about ourselves**: a WARN was parked as impossible for two weeks while six peers in the same census were quietly deciding it, and nobody diffed the severities. Next step is to read a PASS peer against `go` and establish whether the 39 share one defect, per the CAP precedent. |
