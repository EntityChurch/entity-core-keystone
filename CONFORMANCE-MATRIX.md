...
**One row carries a disclosed gap behind a 0-FAIL verdict, named here rather than left to be found.** `cobol` executes 108 of the 756 checks as skips against the cohort's 106: two concurrency probes stage payloads (256 KiB, 16 KiB) that its 65535-byte frame cap and 8192-byte per-entity ceiling cannot accept, and it refuses both with a correlated `413 payload_too_large`. *(This sentence claimed the `413` for both probes from 2026-08-30, and it was only true of the 16 KiB one. The 256 KiB probe exceeds the FRAME cap, and that path drained the body and answered **nothing** — a §4.9(c) silent drop billed to the caller's deadline, which is why `t1_3_no_head_of_line` read as `i/o timeout` and was filed as a capacity skip. §4.10(a) makes the `413` a MUST; it was added 2026-09-02 and the check now reports an honest refusal. The counts did not move — the peer still cannot accept a payload that size, and that bound is spec-legal: §4.10(a) requires a finite maximum and the core protocol places no restriction on entity size.)*
