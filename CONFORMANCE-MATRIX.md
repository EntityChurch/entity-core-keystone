...
| | Peers |
|---|---|
| **0-FAIL @ the 755-check pin** (13) | `go` `haskell` `lean` `ocaml` `swift` (**tier M1**, 5/5) · `common-lisp` `csharp` `elixir` `java` `kotlin` `python` `rust` `typescript` (**tier M2**, 8/8) — M2 completed 2026-08-22 |
...
**What remains is NOT the CAP feature.** `cobol` 30F (the trio plus its standing 27-FAIL liveness cascade), the `asm-x86_64`/`asm-arm64`/`riscv64` trio (INVALID measurements, a connection-pressure family — §1a), `wasm-wat` 2F and `turbowarp` 3F (both hand-authored/exploratory substrates, unstarted). `apl` is unmeasurable and upstream-blocked. Each is its own investigation; none is the mint ceiling.
...
| **wasm-wat** | probe | v0.8.0 | `95edd774…` | 755 · **2F** — 304P/339W/2F/110S | **seam-hybrid** (hand-authored WAT peer + wire codec; Rust codec compiled to `wasm32-wasip1`, wasm-merged as the seam) | **seam** — `entitycore_codec.wasm` (Rust→wasm, Ed25519 + SHA-256) | deferred (→ codec seam `ec_ed448_*`) | source (`make peer`), `0.1.0-pre` |
...
| **`wasm-wat` CAP trio + `turbowarp` CAP-2/3** | 2 peers | Medium — the last two unstarted | The only peers the 2026-08-28 propagation did not reach. `wasm-wat` is hand-authored WebAssembly text: the mint is a literal `(call $w_map (i64.const 4))` with fixed memory offsets, so adding `expires_at` is an arity + data-segment edit rather than a code edit, and the §6.3 salvage needs a second decoder entry point in the same style. `turbowarp` is the block-interpreter probe and never gates (§1 ‡). Neither is hard; both want their own session rather than the tail of a 26-peer batch. |
