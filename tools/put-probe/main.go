// put-probe — measure, on the wire, how a peer admits a `system/tree:put`.
//
// WHY THIS EXISTS. `0.8.2.11` adds §6.3's `put` ADMISSION LADDER — the first
// accept-side rule of the whole 0.8.2.x arc. Everything from `0.8.2.4` through
// `0.8.2.9` was a rule about what a peer EMITS; this one is a rule about what a peer
// ACCEPTS, and architecture flags the consequence directly: an accept-side rule
// "partitions a cohort during adoption rather than merely diverging its error
// strings."
//
// The `tree:put` admission of all 46 generated peers HAS NEVER BEEN DRIVEN — the row
// did not exist until `EXTENSION-TREE` v4.4 and had no predicate until v4.5. Arch's
// instruction is explicit and is followed rather than softened: "Do not size the work
// from the assumption that they are conformant and this is a re-vendor." So this
// measures first, and the worklist item is sized from the result.
//
// THE LADDER (`ENTITY-CORE-PROTOCOL.md` §6.3, 0.8.2.11), two ordered steps:
//
//	1. STRUCTURE. The submitted value is an entity when it is a map carrying a
//	   non-empty text `type`, a present `data` (any CBOR value — null is legal), and a
//	   `content_hash` that is a well-formed `system/hash` whose byte length matches its
//	   format code. Any failure -> 400 `invalid_request`.
//	2. HASH. Carried `content_hash` vs `content_hash({type, data})`. Disagreement ->
//	   400 `hash_mismatch`.
//
// Step 1 strictly precedes step 2. The spec states why that ordering cannot be tested
// the obvious way: "no vector carrying a single fault can discriminate the order",
// since each row's own input reaches its own branch either way. The discriminating
// input carries BOTH faults at once, and that is case E below.
//
// THE ONE ARCH ASKED FOR FIRST is case B — a two-key entity `{type, data}` with no
// `content_hash`. "That single input separates the three behaviours the ground-up
// cohort exhibits today (refuse correctly · refuse with the wrong code · SILENTLY
// AUTHOR AND STORE)." The third is a correctness defect rather than a code-spelling
// one: a peer MUST NOT author a submitted entity's `content_hash` on the submitter's
// behalf, because it then holds an entity under a hash nobody agreed to. Case B is
// therefore followed by a GET on the same path, since a peer that answers 200 and a
// peer that answers 200 having stored nothing are different findings.
//
// CONTROLS, both mandatory, both learned the hard way by `tools/p47-probe`:
//
//   - POSITIVE. Case A is a valid `put` that MUST answer 200. It runs first, and if it
//     does not pass, every other result on that peer is reported `trusted: false` and
//     MUST NOT be read as a §6.3 answer — the fault is ours. `put_error_control_valid_200`
//     exists upstream for exactly this reason, so the shape is blessed.
//   - DIFFERENTIAL. The ordering question is STATE-shaped rather than value-shaped, so
//     single-fault inputs provably cannot discriminate it (the spec says so). Cases C
//     and D isolate one fault each; case E carries both. C+D+E together are the
//     differential — E's answer is only meaningful against the two single-fault arms,
//     because a peer that answers `invalid_request` to EVERYTHING trivially "passes" E.
//
// A probe without both is a rumour with a number attached.
//
// This is a MEASUREMENT, not a gate. It always exits 0. The oracle's own `put_*`
// checks arrive with the re-pin and are what will gate this surface; this exists to
// size the work before the vendor step, which is the order arch asked for.
//
...
//
//	put-probe -addr 127.0.0.1:7777 [-json-out report.json] [-peer name] [-dump]
package main
...
var cases = []caseSpec{
	{"A_control_valid", "local/putprobe/a", entValid, 200, "",
		"POSITIVE CONTROL — a valid put MUST answer 200, or nothing else here is trustworthy"},
	{"B_absent_content_hash", "local/putprobe/b", entNoHash, 400, "invalid_request",
		"the two-key {type,data} form; 200 here means the peer AUTHORED the submitter's hash"},
	{"C_empty_type", "local/putprobe/c", entEmptyType, 400, "invalid_request",
		"step-1 fault alone (hash is correct for the empty-typed value)"},
	{"D_hash_mismatch", "local/putprobe/d", entHashMismatch, 400, "hash_mismatch",
		"step-2 fault alone (structurally perfect, wrong hash)"},
	{"E_both_faults_ordering", "local/putprobe/e", entBothFaults, 400, "invalid_request",
		"ORDERING DISCRIMINATOR — both faults; hash_mismatch means step 2 ran before step 1"},
	{"F_unsupported_hash_format", "local/putprobe/f", entUnsupportedFormat, 400, "unsupported_content_hash_format",
		"the separate §1.2 row, not the step-1 row"},
}

type caseResult struct {
	ID       string `json:"id"`
	Path     string `json:"path"`
	WantSt   int    `json:"want_status"`
...
type report struct {
	...
	StoredB  string       `json:"b_stored_after_refusal"`
	Ordering string       `json:"ordering_verdict"`
	Summary  string       `json:"summary"`
	// DedupDropped is how many duplicate map keys the ENCODER discarded while
	// building this peer's frames. It is reported unconditionally: a silent
	// dedup would hide the defect it exists to prevent.
	DedupDropped int         `json:"encoder_duplicate_keys_dropped"`
	Diag     *diagnostics `json:"diagnostics,omitempty"`
}

// diagnostics is emitted ONLY when the positive control fails, because that is the
// case where the reader needs to tell a probe bug from a peer answer and the status
// alone cannot.
...
func verdictOf(c caseSpec, st int, code string, err error) string {
	switch {
	case err != nil:
		return "ERROR"
	case st == 0:
		return "NO-RESPONSE (§4.9(c) silent drop)"
	case st == c.wantSt && (c.wantCode == "" || code == c.wantCode):
		return "CONFORMANT"
	case st == c.wantSt:
		return fmt.Sprintf("WRONG-CODE (got %q, want %q)", code, c.wantCode)
	case c.wantSt == 400 && st == 200:
		return "ACCEPTED — the peer admitted a value §6.3 requires it to refuse"
	default:
		return fmt.Sprintf("WRONG-STATUS (got %d %s, want %d %s)", st, code, c.wantSt, c.wantCode)
	}
}

func main() {
	// Args are parsed BY HAND, tolerating unknown flags, because this binary is
	// dropped into every peer's run-s4.sh via ORACLE= and therefore receives whatever
	// the harness passes the real validator (`-profile core -json-out <path>`,
	// possibly `-reference-peer <addr>`). The flag package would abort on those.
	...
	// Every case gets a FRESH session. A peer that answers a refusal by closing the
	// connection (the §6.3 transport-drop defect this repo has recorded five times)
	// would otherwise cascade into every later case and be reported as six findings
	// instead of one.
	for _, c := range cases {
		cr := caseResult{ID: c.id, Path: c.path, WantSt: c.wantSt, WantCode: c.wantCode, Note: c.note}
		...
		env, err := s.authedExecute(c.id, "system/tree", "put", putParams(c.build()), []string{c.path})
		s.close()
		if err != nil {
			cr.Err = err.Error()
		} else {
			cr.Status, cr.Code, _ = statusOf(env)
			cr.Message = messageOf(env)
		}
		cr.Verdict = verdictOf(c, cr.Status, cr.Code, err)
		r.Cases = append(r.Cases, cr)
	}

	// The positive control decides whether any of the above may be read at all.
	for _, cr := range r.Cases {
		if cr.ID == "A_control_valid" {
			r.Trusted = cr.Status == 200
			r.Control = fmt.Sprintf("valid put -> %d %s", cr.Status, cr.Code)
			if !r.Trusted {
				r.Control += " | UNTRUSTED: the probe could not complete a legal put on this " +
					"peer, so every other row is a probe-side fault and NOT a §6.3 answer"
	...
	// Did case B's value actually land? A 200 and a 200-having-stored-nothing are
	// different findings, and "silently author and store" is the one that matters.
	r.StoredB = "not-checked"
	if s, err := dialSession(addr, to); err == nil {
		env, err := s.authedExecute("bget", "system/tree", "get", emptyParams(),
			[]string{"local/putprobe/b"})
		s.close()
		if err == nil {
			st, code, _ := statusOf(env)
			switch {
			case st == 200:
				r.StoredB = "STORED — the peer authored a content_hash the submitter never supplied"
			case st == 404:
				r.StoredB = "absent (404) — nothing was bound"
			default:
				r.StoredB = fmt.Sprintf("inconclusive (%d %s)", st, code)
			}
		} else {
			r.StoredB = "inconclusive (" + err.Error() + ")"
		}
	}

	// The ordering verdict is only meaningful when both single-fault arms answered,
	// which is what makes C and D controls rather than extra data points.
	var cSt, dSt, eSt int
	var cCode, dCode, eCode string
	for _, cr := range r.Cases {
		switch cr.ID {
		case "C_empty_type":
			cSt, cCode = cr.Status, cr.Code
		case "D_hash_mismatch":
			dSt, dCode = cr.Status, cr.Code
		case "E_both_faults_ordering":
			eSt, eCode = cr.Status, cr.Code
		}
	}
	switch {
	case !r.Trusted:
		r.Ordering = "untrusted — control did not pass"
	case cSt != 400 || dSt != 400:
		r.Ordering = fmt.Sprintf("INDETERMINATE — the single-fault arms do not both refuse "+
			"(C=%d %s, D=%d %s), so E cannot discriminate an ORDER between two steps this "+
			"peer does not both implement", cSt, cCode, dSt, dCode)
	case cCode == dCode:
		r.Ordering = fmt.Sprintf("INDETERMINATE — this peer answers %q to both single faults, "+
			"so no answer to E can distinguish which step ran first", cCode)
	case eCode == cCode:
		r.Ordering = "step 1 precedes step 2 (conformant)"
	case eCode == dCode:
		r.Ordering = "STEP 2 RAN FIRST — E answered the hash row for an input that fails structure"
	default:
		r.Ordering = fmt.Sprintf("unclassified (E=%d %s against C=%q D=%q)", eSt, eCode, cCode, dCode)
	}

	conf := 0
	for _, cr := range r.Cases {
		if cr.Verdict == "CONFORMANT" {
			conf++
		}
	}
	r.Summary = fmt.Sprintf("%d/%d conformant", conf, len(r.Cases))
	...
	// Always 0: this measures, it does not gate. The oracle's put_* checks gate.
}
