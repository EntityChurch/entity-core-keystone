// wire.go — canonical ECF, framing, and an authenticated session.
//
// Lifted from tools/host-seam-probe (Kind A probe), whose encoder, decoder and handshake have
// round-tripped against all 46 peers. The only changes: the session takes the identity seed as a
// parameter (the contract suite drives four identities), and request ids are tagged `kpc-`.
// Keep the two copies' codec halves in step by hand until one of them is retired — this is a
// second copy on purpose, because the probe is a measurement instrument that must not change
// under a suite that is still a draft.
package main
...
func keyOf(seed byte) ed25519.PrivateKey {
...
type session struct {
	...
	remotePeerID string // the responder's peer_id as its hello answered
	seq          int
...
func (s *session) rid(tag string) string {
	s.seq++
	return fmt.Sprintf("kpc-%s-%d", tag, s.seq)
}
...
func dialSession(addr string, seed byte, to time.Duration) (*session, error) {
	...
	s.remotePeerID, _ = resultField(env, "peer_id").(string)
